...
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_jgdy_em.py)
define a full A-share response with the exact 13 source-shaped fields `序号`,
`代码`, `名称`, `最新价`, `涨跌幅`, `调研机构`, `机构类型`, `调研人员`,
`接待方式`, `接待人员`, `接待地点`, `调研日期` and `公告日期`.

The provider validates the complete response before filtering it to the
requested A-share listing. It enforces exact field order, six-digit code
identity, valid ISO research/announcement dates, the official strict
`调研日期 > date` boundary, strictly ascending source sequence, finite
...
`(PFORG_TYPE="证券")`, requested in one 500-row page ordered by descending
`ORG_NUM`, and returned in the exact eight-field source order `序号`, `质押机构`,
`质押公司数量`, `质押笔数`, `质押数量`, `未达预警线比例`,
...
count types, 0–100 documented percentage bounds and non-nullability. It
records that `质押数量` is in shares and the three risk-state fields are in
...
### Phase 3.09 — A-share Legu market-activity raw acquisition contract (COMPLETE)
...
validated as finite and non-negative; `活跃度` remains provider text and
`统计日期` is validated as a strict `YYYY-MM-DD HH:MM:SS` timestamp. No units
...
### Phase 3.13 — A-share Legu TTM/LYR PE raw acquisition contract (COMPLETE)
...
### Phase 3.14 — A-share Legu all-PB raw acquisition contract (COMPLETE)
...
### Phase 3.17 — A-share Legu index PE raw acquisition contract (COMPLETE)
...
The [AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_a_pe_and_pb.py)
define the exact eight-field `日期`/`指数`/static-PE/rolling-PE output. All
...
### Phase 3.18 — A-share Legu index PB raw acquisition contract (COMPLETE)
...
define the exact five-field `日期`/`指数`/PB output. All symbols use the Legu
...
### Phase 3.19 — A-share Baidu valuation raw acquisition contract (COMPLETE)
...
### Phase 3.23 — A-share Eastmoney growth-comparison raw acquisition contract (COMPLETE)
...
### Phase 3.24 — H-share Eastmoney growth-comparison raw acquisition contract (COMPLETE)
...
mapped `基本每股收总资产同比增长率益同比增长率` label, four nullable/signed
...
### Phase 3.29 — H-share Stock Connect constituent quote raw acquisition contract (COMPLETE)
...
`pz=100`, `fid=f12` and the exact 12-field wrapper order `序号`, `代码`, `名称`,
...
### Phase 3.32 — HSGT individual-ranking raw acquisition contract (COMPLETE)
...
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hsgt_em.py)
define the A-share market choices `北向`, `沪股通` and `深股通`, plus the
period choices `今日排行`, `3日排行`, `5日排行`, `10日排行`, `月排行`, `季排行`
and `年排行`. The provider maps the two directional markets to
...
the five `增持估计` fields use the selected period prefix, and preserves
...
### Phase 3.36 — HSGT historical-flow raw acquisition contract (COMPLETE)

The mapping review now covers the documented Eastmoney
`stock_hsgt_hist_em` HSGT historical-flow endpoint under
`CAPITAL_FLOW` with explicit `view=hsgt_hist` and
the six official symbols `北向资金`, `沪股通`,
`深股通`, `南向资金`, `港股通沪` and
`港股通深`. The [AKShare stock-data
documentation](https://akshare.akfamily.xyz/data/stock/stock.html) and
[official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hsgt_em.py)
...
### Phase 3.37 — HSGT individual-detail raw acquisition contract (COMPLETE)
...
### Phase 3.39 — Shanghai A-share quote raw acquisition contract (COMPLETE)
...
### Phase 3.41 — Beijing A-share quote raw acquisition contract (COMPLETE)
...
### Phase 3.42 — A-share new-stock quote raw acquisition contract (COMPLETE)
...
exact 24-field wrapper order, including `上市日期`, the 33-column source
...
### Phase 3.43 — Growth Enterprise Market quote raw acquisition contract (COMPLETE)
...
### Phase 3.44 — STAR Market quote raw acquisition contract (COMPLETE)
...
### Phase 3.45 — B-share quote raw acquisition contract (COMPLETE)
...
### Phase 3.47 — Sina B-share daily-history raw acquisition contract (COMPLETE)
...
### Phase 3.63 — Sina Hong Kong-index spot raw acquisition contract (COMPLETE)
...
[`docs/goals/phase-5r-a-production-source-acquisition.md`](goals/phase-5r-a-production-source-acquisition.md).
The project, rather than the user, must acquire selected
...
Phase 5R-A is personal-first. Official public and documented personal-account
sources are the default; institutional market-data contracts are optional and
may not be a prerequisite for the minimum path. The first exit gates are an
explicitly acquired local raw corpus, a named bounded A/H target, source-specific
coverage and access evidence, no current-constituent substitution, honest
listing/lifecycle and terminal handling, independent sampled reconciliation,
...
`EventImpactDecisionV1`/`ReanalysisRequestV1`/`ReanalysisPlanV1`/
`MonitoringRunV1` contracts, the versioned monitoring-only `event-impact-v1`
policy (isolated from `strict-v1`), point-in-time filtering on the canonical
event availability boundary, deterministic canonical ordering by
`(available_at, source_id, event_id, listing_id)`, idempotent duplicate-event
handling with hard failure on conflicting content for the same event
identity, source/listing-scoped cursors that advance only through the
...
values, `活跃度` remains provider text, and `统计日期` is a strict
