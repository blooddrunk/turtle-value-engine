...
> Closure status: Phase 2 provider-backed preparation and the top-level Phase
> 3 filing-to-decision integration are complete. The endpoint catalog below
> remains historical component documentation.
...
interpretation and LLM analysis remain outside the provider/cache layer. The
top-level closure is composed by the provider-backed preparation boundary,
the explicit accepted-adjustment materializer and the deterministic decision
...
### Provider-backed preparation boundary

`AcquisitionRequest`, `AcquisitionBundle` and
`NormalizedCompanyInputBuilder` form the stable Phase 2 composition boundary.
The request canonicalizes the A/H listing, binds an explicit `as_of` date and
selects categories/providers. The builder acquires each category through the
existing raw-response cache, validates retrieval and declared observation or
publication dates, and then calls an injected normalizer. The default AKShare
path returns the existing `NormalizedCompanyInput`; provider field names stop
at the raw-record/normalizer boundary.

The preparation layer requires explicit company context (or an injected
resolver), preserves missing fields and unresolved units/periods, and rejects
ambiguous normalization rather than guessing. Its only boundary-specific
value transform is the documented conversion of signed cash-paid provider
lines to the positive outflow magnitudes required by the frozen CDC input
contract. Raw cache bytes and deterministic calculation code are not rewritten.
`tve prepare` is the user-facing live-or-replay entry point. `tve analyze`
still accepts a frozen normalized artifact and performs no provider import or
network operation.

### Accepted adjustments and traceability boundary

`materialize_effective_input` is the only composition boundary that turns an
adjustment into an analysis-effective fact. It applies only `ACCEPTED`
adjustments with `HUMAN` or `RULE_ENGINE` approval; `LLM` remains a proposer
only. Targets are restricted to canonical numeric source-side facts, and
stale input values, conflicting same-fact replacements, missing evidence or
unsupported targets fail closed. The original fact remains in the source
snapshot; the replacement fact carries `source_fact_id`, `source_value` and
`applied_adjustment_ids`, with evidence references unioned for auditability.

`run_analyze_with_accepted_adjustments` materializes first and then delegates
to the unchanged offline `run_analyze` pipeline. `build_decision_trace`
projects the validated result into deterministic nodes and links so the
decision can be followed through gate, metric, effective/source fact,
...
## 12. Phase 2.2–3.83 AKShare adapter
...
individual-holdings/individual-detail, individual-ranking, daily-stock-statistics and institution-statistics raw slices, A-share Eastmoney individual-fund-flow,
...
| `MARKET_QUOTE` | `stock_zh_a_spot_em`; `stock_sh_a_spot_em` (`view=sh_a_spot`, full Shanghai A-share universe filtered by requested Shanghai listing); `stock_sz_a_spot_em` (`view=sz_a_spot`, full Shenzhen A-share universe filtered by requested Shenzhen listing); `stock_bj_a_spot_em` (`view=bj_a_spot`, full Beijing A-share universe filtered by requested Beijing listing); `stock_new_a_spot_em` (`view=new_a_spot`, full A-share new-stock universe filtered by requested A-share listing); `stock_cy_a_spot_em` (`view=cy_a_spot`, full Growth Enterprise Market universe filtered by requested Growth Enterprise Market listing); `stock_kc_a_spot_em` (`view=kc_a_spot`, full STAR Market universe filtered by requested STAR Market listing); `stock_zh_kcb_spot` (`view=kcb_sina_spot`, full Sina STAR Market universe filtered by requested STAR Market listing); `stock_zh_b_spot_em` (`view=b_spot`, full B-share universe filtered by requested Shanghai 900xxx or Shenzhen 200xxx listing); `stock_zh_index_spot_em` (`view=index_spot`, full Eastmoney mainland-index universe with the documented category selector); `stock_bid_ask_em` (`view=bid_ask`, Shanghai/Shenzhen A-share only); `stock_individual_spot_xq` (`view=xueqiu_spot`, symbol-scoped A-share quote); `stock_zh_ah_spot_em` (`view=ah_comparison`, full A+H universe filtered by requested A/H side); `stock_zh_ab_comparison_em` (`view=ab_comparison`, full A+B universe filtered by requested A-share side) | `stock_hk_spot_em` (`view=hk_spot_em`, full 15-minute-delayed Eastmoney H-share universe filtered by requested H-share listing); `stock_hk_index_spot_sina` (`view=hk_index_spot_sina`, full Sina Hong Kong-index universe; H-share listing context only); `stock_hk_index_spot_em` (`view=hk_index_spot_em`, full Eastmoney Hong Kong-index universe; H-share listing context only); `stock_hk_main_board_spot_em` (`view=hk_main_board`, full H-share main-board universe filtered by requested H-share); `stock_hk_famous_spot_em` (`view=hk_famous`, full H-share famous-stock universe filtered by requested H-share); `stock_hk_ggt_components_em` (`view=hk_ggt_components`, full H-share Stock Connect constituent universe filtered by requested H-share); `stock_hsgt_sh_hk_spot_em` (`view=hk_sh_spot`, full Shanghai Stock Connect H-share universe filtered by requested H-share); `stock_zh_ah_spot_em` (`view=ah_comparison`, full A+H universe filtered by requested H-share side) | selected-listing `current_price` plus quote timestamp from the existing quote contract; Xueqiu current price/timestamp use the existing quote contract while Eastmoney H-share, other Xueqiu fields, bid/ask, Shanghai A-share, Shenzhen A-share, Beijing A-share, A-share new-stock, Growth Enterprise Market, STAR Market, Sina STAR Market, B-share, Sina mainland-index spot, Sina Hong Kong-index spot, Eastmoney mainland-index spot, Eastmoney Hong Kong-index spot, H-share main-board, H-share famous-stock, H-share Stock Connect constituent, HSGT Shanghai Stock Connect, A+B and A+H comparison views remain raw-only |
| `MARKET_HISTORY` | `stock_zh_a_hist` (daily history); `stock_zh_a_daily` (`view=sina_a_daily`, explicit inclusive date range/adjustment; A-share only); `stock_intraday_em` (`view=intraday_trades`, latest-trading-day time-only trades; A-share only); `stock_intraday_sina` (`view=intraday_sina`, requested-date time-only large-order rows; A-share only); `stock_cyq_em` (`view=chip_distribution`, latest 90 trading days and adjustment; A-share only); `stock_zh_a_hist_tx` (`view=tencent_daily`, explicit date range/adjustment; A-share only); `stock_zh_index_daily_tx` (`view=tencent_index_daily`, Shanghai 000xxx or Shenzhen 399xxx index date range; A-share index context only); `stock_zh_index_daily_em` (`view=index_daily_em`, Shanghai 000xxx, Shenzhen 399xxx or Beijing 899xxx index date range; A-share index context only); `index_zh_a_hist` (`view=index_zh_a_hist`, Shanghai 000xxx, Shenzhen 399xxx or Beijing 899xxx index period/date range; A-share index context only); `index_zh_a_hist_min_em` (`view=index_zh_a_hist_min_em`, Shanghai 000xxx, Shenzhen 399xxx or Beijing 899xxx index period/datetime range; A-share index context only); `stock_zh_a_tick_tx_js` (`view=tencent_tick`, latest-trading-day time-only ticks; A-share only); `stock_zh_a_minute` (`view=sina_minute`, explicit interval/adjustment; A-share only); `stock_zh_a_hist_min_em` (`view=intraday`, explicit datetime range/interval/adjustment; A-share only); `stock_zh_a_hist_pre_min_em` (`view=pre_market`, explicit time-of-day range; A-share only); `stock_zh_b_daily` (`view=b_daily`, B-share listing-scoped date range and adjustment; A-share listing context only); `stock_zh_b_minute` (`view=b_minute`, B-share listing-scoped recent minute window and adjustment; A-share listing context only); `index_global_hist_em` (`view=global_index_hist`, explicit documented `index_symbol`, full Eastmoney global-index history; A-share listing context only); `index_global_hist_sina` (`view=global_index_hist_sina`, explicit documented `index_symbol`, recent Sina global-index history; A-share listing context only) | `stock_hk_daily`; `stock_zh_ah_daily` (`view=ah_daily`, explicit year range/adjustment; H-share listing context only); `stock_hk_index_daily_sina` (`view=hk_index_daily_sina`, explicit `index_symbol`, full Sina Hong Kong-index history; H-share listing context only); `stock_hk_index_daily_em` (`view=hk_index_daily_em`, explicit `index_symbol`, full Eastmoney Hong Kong-index history; H-share listing context only); `index_global_hist_em` (`view=global_index_hist`, explicit documented `index_symbol`, full Eastmoney global-index history; H-share listing context only); `index_global_hist_sina` (`view=global_index_hist_sina`, explicit documented `index_symbol`, recent Sina global-index history; H-share listing context only); `stock_hk_hist_min_em` (`view=hk_intraday`, explicit datetime range/interval/adjustment; H-share only) | dated daily OHLCV/turnover extension facts from standard and Tencent daily history; Sina A-share, Tencent A+H, Tencent, Eastmoney and generic Eastmoney index daily-history, Eastmoney index minute-history, Sina and Eastmoney Hong Kong-index daily-history, Eastmoney global-index daily-history, Sina global-index daily-history, Eastmoney/Sina intraday-trade, chip-distribution, Tencent tick, Sina minute, A-share/H-share intraday, latest-day pre-market, B-share daily-history and B-share minute-history rows as raw structured evidence only |
| `MARKET_ACTIVITY` | `stock_zh_a_new_em` (`view=new_stock`, current-trading-day new-stock universe; A-share only); `stock_zh_a_new` (`view=sina_new_stock`, latest-trading-day next-new-stock universe; Shanghai/Shenzhen A-share only); `stock_gsrl_gsdt_em` (`view=company_dynamics`, requested trading-day company-dynamics universe; A-share only); `stock_xgsr_ths` (`view=new_stock_first_day`, historical new-stock first-day universe; A-share only); `stock_comment_detail_scrd_desire_em` (`view=participation_desire`, latest 30 trading days; A-share only); `stock_comment_detail_scrd_focus_em` (`view=focus`, latest 30 trading days; A-share only); `stock_comment_detail_zlkp_jgcyd_em` (`view=institution_participation`, symbol-scoped historical series; A-share only); `stock_hot_rank_em` (`view=hot_rank`, current-trading-day top 100; A-share only); `stock_hot_rank_latest_em` (`view=hot_rank_latest`, symbol-scoped latest rank; A-share only); `stock_hot_rank_detail_em` (`view=hot_rank_detail`, symbol-scoped recent historical dates; A-share only); `stock_hot_rank_detail_realtime_em` (`view=hot_rank_detail_realtime`, symbol-scoped recent intraday rank rows; A-share only); `stock_zt_pool_em` (`view=limit_up_pool`, requested `date` limit-up pool; A-share only); `stock_zt_pool_dtgc_em` (`view=limit_down_pool`, requested `date` limit-down pool; A-share only); `stock_lhb_detail_em` (inclusive `start_date`/`end_date`; A-share only); `stock_lhb_stock_statistic_em` (`view=stock_statistic`, explicit `period`; A-share only); `stock_lhb_jgstatistic_em` (`view=institution_statistic`, explicit `period`; A-share only); `stock_dzjy_mrmx` (`view=block_trade_detail`, A-share `symbol="A股"` and inclusive date range; full universe filtered to requested listing); `stock_lhb_jgmmtj_em` (`view=institution_daily`, inclusive date range; full universe filtered to requested listing); `stock_jgdy_tj_em` (`view=institution_research`, strict `公告日期` after requested `date`; full universe filtered to requested listing); `stock_jgdy_detail_em` (`view=institution_research_detail`, strict `调研日期` after requested `date`; full universe filtered to requested listing); `stock_board_industry_name_em` (`view=industry_board`, current industry-board snapshot; A-share only); `stock_hsgt_board_rank_em` (`view=hsgt_board_rank`, three northbound board selectors and seven documented periods; A-share only); `stock_szse_sector_summary` (`view=szse_sector_summary`, requested `symbol=当月` or `当年` and `date=YYYYMM` industry summary; A-share only); `stock_szse_area_summary` (`view=szse_area_summary`, requested `date=YYYYMM` region-ranked market summary; A-share only); `stock_szse_summary` (`view=szse_summary`, requested `date=YYYYMMDD` security-category market summary; A-share only); `stock_sse_summary` (`view=sse_summary`, latest market summary; A-share only); `stock_sse_deal_daily` (`view=sse_deal_daily`, requested `date=YYYYMMDD` overview; A-share only); `stock_account_statistics_em` (`view=account_statistics`, no upstream arguments, market-wide monthly history; A-share only); `stock_market_activity_legu` (`view=market_activity_legu`, no upstream arguments, current snapshot; A-share only); `stock_a_congestion_lg` (`view=congestion`, no upstream arguments, latest-four-year market history; A-share only); `stock_ebs_lg` (`view=equity_bond_spread`, no upstream arguments, all historical market context; A-share only); `stock_buffett_index_lg` (`view=buffett_index`, no upstream arguments, all historical market-capitalization/GDP context; A-share only); `stock_a_ttm_lyr` (`view=ttm_lyr`, no upstream arguments, all historical A-share TTM/LYR PE context; A-share only); `stock_a_all_pb` (`view=all_pb`, no upstream arguments, all historical A-share PB context; A-share only); `stock_index_pe_lg` (`view=index_pe`, required `symbol` in the 12 documented index choices, all historical index-level PE context; A-share only); `stock_index_pb_lg` (`view=index_pb`, required `symbol` in the 12 documented index choices, all historical index-level PB context; A-share only); `stock_market_pe_lg` (`view=market_pe`, required `symbol` in `上证`/`深证`/`创业板`/`科创版`, all historical board/index PE context; A-share only); `stock_market_pb_lg` (`view=market_pb`, required `symbol` in `上证`/`深证`/`创业板`/`科创版`, all historical board/index PE context; A-share only); `stock_zh_valuation_baidu` (`view=valuation_baidu`, listing-scoped indicator/period history; A-share only) | `stock_hk_hot_rank_latest_em` (`view=hot_rank_latest`, symbol-scoped latest rank; H-share only); `stock_hk_hot_rank_detail_em` (`view=hk_hot_rank_detail`, symbol-scoped historical dates; H-share only); `stock_hk_hot_rank_detail_realtime_em` (`view=hk_hot_rank_detail_realtime`, symbol-scoped recent intraday rank rows; H-share only); `stock_hk_valuation_baidu` (`view=valuation_baidu_hk`, listing-scoped indicator/period history; H-share only) | A-share/H-share latest stock-popularity rank, H-share realtime stock-popularity rank, A-share/H-share historical stock-popularity rank, A-share realtime stock-popularity rank and the existing A-share new-stock-board, Sina next-new-stock, company-dynamics, new-stock-first-day, market-participation-desire, market-focus, institution-participation, limit-up-pool, limit-down-pool, Dragon-Tiger detail, per-listing statistics, institution-seat statistics, institution-daily, institutional-research, institutional-research-detail or block-trade detail rows plus Eastmoney industry-board and HSGT board-rank, SSE/SZSE market-summary, SZSE area-summary/sector-summary, market-wide stock-account-statistics, Legu market-activity, congestion, equity-bond-spread, Buffett-index, TTM/LYR PE, PB, index-PE, index-PB, market-PE, market-PB and A/H Baidu valuation-history aggregates retained as raw structured evidence only; no issuer cash-flow, shareholder-return, governance, market or valuation fact |
| `MARKET_ACTIVITY` (IPO benefit) | `stock_ipo_benefit_ths` (`view=ipo_benefit`, historical weekly IPO-benefit universe; A-share only) | — | IPO-benefit stock rows retained as raw structured evidence only; no canonical revenue, cash-flow, ownership, valuation or accounting fact |
| `MARKET_ACTIVITY` (two-net/delisted stocks) | `stock_zh_a_stop_em` (`view=stop_stock`, current-trading-day two-net and delisted-stock universe; A-share only) | — | two-net/delisted-stock quote and provider-universe rows retained as raw structured evidence only; no canonical listing, price, valuation or accounting fact |
| `MARKET_QUOTE` (Sina KCB) | `stock_zh_kcb_spot` (`view=kcb_sina_spot`, current-trading-day full Sina STAR Market quote universe; Shanghai 688xxx/689xxx A-share only) | — | Sina STAR Market quote rows, including provider observation time and quote/valuation fields, retained as raw structured evidence only; no canonical current-price fact |
| `MARKET_QUOTE` (Sina index spot) | `stock_zh_index_spot_sina` (`view=index_spot_sina`, current-trading-day full Sina mainland-index universe; A-share listing context only) | — | Sina index-universe quote rows retained as raw structured evidence only; no canonical current-price fact |
| `MARKET_QUOTE` (Sina Hong Kong-index spot) | — | `stock_hk_index_spot_sina` (`view=hk_index_spot_sina`, current-trading-day full Sina Hong Kong-index universe; H-share listing context only) | Sina Hong Kong-index quote rows retained as raw structured evidence only; no canonical current-price fact |
| `MARKET_QUOTE` (Sina H-share realtime) | — | `stock_hk_spot` (`view=hk_spot_sina`, current-trading-day 15-minute-delayed full Sina H-share universe filtered by requested H-share listing) | Sina H-share realtime quote rows, timestamps and bid/ask context retained as raw structured evidence only; no canonical current-price fact |
| `MARKET_QUOTE` (Eastmoney H-share realtime) | — | `stock_hk_spot_em` (`view=hk_spot_em`, current-trading-day 15-minute-delayed full Eastmoney H-share universe filtered by requested H-share listing) | Eastmoney H-share realtime quote rows retained as raw structured evidence only; no canonical current-price fact |
| `MARKET_QUOTE` (Eastmoney global-index spot) | `index_global_spot_em` (`view=global_index_spot`, current global-index universe; A-share listing context only) | `index_global_spot_em` (`view=global_index_spot`, current global-index universe; H-share listing context only) | Eastmoney global-index quote rows retained as raw structured evidence only; no canonical current-price fact |
| `MARKET_QUOTE` (Eastmoney Hong Kong-index spot) | — | `stock_hk_index_spot_em` (`view=hk_index_spot_em`, current-trading-day full Eastmoney Hong Kong-index universe; H-share listing context only) | Eastmoney Hong Kong-index quote rows retained as raw structured evidence only; no canonical current-price fact |
...
| `MARKET_HISTORY` (Sina Hong Kong-index daily history) | — | `stock_hk_index_daily_sina` (`view=hk_index_daily_sina`, explicit `index_symbol`, full six-field history; H-share listing context only) | Sina Hong Kong-index OHLCV rows retained as raw structured evidence only; no canonical daily market-history, return, valuation or accounting fact |
| `MARKET_HISTORY` (Eastmoney Hong Kong-index daily history) | — | `stock_hk_index_daily_em` (`view=hk_index_daily_em`, explicit `index_symbol`, full five-field history; H-share listing context only) | Eastmoney Hong Kong-index OHLC rows retained as raw structured evidence only; no canonical daily market-history, return, valuation or accounting fact |
| `MARKET_HISTORY` (Sina US-index daily history) | `index_us_stock_sina` (`view=us_index_sina`, explicit `index_symbol` in `.INX`/`.IXIC`/`.DJI`/`.NDX`, full seven-field history; A/H listing context only) | `index_us_stock_sina` (`view=us_index_sina`, explicit `index_symbol` in `.INX`/`.IXIC`/`.DJI`/`.NDX`, full seven-field history; A/H listing context only) | Sina US-index OHLCV/amount rows retained as raw structured evidence only; no canonical daily market-history, return, valuation or accounting fact |
| `MARKET_HISTORY` (Eastmoney global-index daily history) | `index_global_hist_em` (`view=global_index_hist`, explicit `index_symbol` in documented global-index names, full eight-field history; A-share listing context only) | `index_global_hist_em` (`view=global_index_hist`, explicit `index_symbol` in documented global-index names, full eight-field history; H-share listing context only) | Eastmoney global-index OHLC/amplitude rows retained as raw structured evidence only; no canonical daily market-history, return, valuation or accounting fact |
| `MARKET_HISTORY` (Tencent index daily history) | `stock_zh_index_daily_tx` (`view=tencent_index_daily`, Shanghai 000xxx or Shenzhen 399xxx index date range) | — | Tencent front-adjusted index OHLC and lot-amount rows retained as raw structured evidence only; no canonical daily market-history, return, valuation or accounting fact |
| `MARKET_HISTORY` (Eastmoney index daily history) | `stock_zh_index_daily_em` (`view=index_daily_em`, Shanghai 000xxx, Shenzhen 399xxx or Beijing 899xxx index date range) | — | Eastmoney unadjusted index OHLCV/amount rows retained as raw structured evidence only; no canonical daily market-history, return, valuation or accounting fact |
| `MARKET_HISTORY` (generic Eastmoney index history) | `index_zh_a_hist` (`view=index_zh_a_hist`, Shanghai 000xxx, Shenzhen 399xxx or Beijing 899xxx index period/date range) | — | Eastmoney unadjusted multi-period index OHLCV, turnover and provider-derived change rows retained as raw structured evidence only; no canonical daily market-history, return, valuation or accounting fact |
| `MARKET_HISTORY` (Eastmoney index minute history) | `index_zh_a_hist_min_em` (`view=index_zh_a_hist_min_em`, Shanghai 000xxx, Shenzhen 399xxx or Beijing 899xxx index period/datetime range) | — | Eastmoney index intraday bars retained as raw structured evidence only; no canonical daily market-history, return, valuation or accounting fact |
...
The H-share `stock_hk_hot_rank_detail_realtime_em` view is the adjacent
documented symbol-scoped realtime-rank endpoint selected only with
`view=hk_hot_rank_detail_realtime`. It passes the unprefixed five-digit code to
the provider's `getCurrentHkUsList` JSON POST, validates the exact two-field
`时间`/`排名` response and strictly ascending timestamp order, and records the
`HK|` security code, fixed `marketType=000003`, upstream request contract,
wrapper mapping and observation-time bounds for replay. The normalizer emits
`AKSHARE_HK_HOT_RANK_DETAIL_REALTIME_RAW_ONLY`; the intraday provider
popularity rank remains raw evidence and does not become a canonical market,
issuer cash-flow, shareholder-return, governance or valuation input.
...
The H-share Eastmoney valuation-comparison slice is also acquisition-only. The
current [AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
...
valuation-comparison row. The adapter exposes it only under `MARKET_ACTIVITY`
with explicit `view=valuation_comparison_hk`; the requested H-share listing
supplies the unprefixed five-digit upstream `symbol` and Eastmoney
`(SECUCODE="<code>.HK")(CORRE_SECUCODE="<code>.HK")` filter. The current
wrapper emits the exact 18 fields `代码`, `简称`, eight valuation multiples and
eight corresponding integer ranks. The provider freezes the explicit upstream
column list, report/filter/client parameters, records the one-row listing
scope and validates the exact field order, listing identity, nullable/signed
finite multiples and positive integer ranks.

The normalizer emits `AKSHARE_HK_VALUATION_COMPARISON_RAW_ONLY`; the
provider-defined H-share comparison remains raw evidence only and creates no
canonical valuation, market, return, governance or accounting fact. A-share
requests and any calculation, gate, pipeline, CLI or input-loader use remain
outside this slice.

The A-share Eastmoney growth-comparison slice is also acquisition-only. The
current [AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock/stock_zh_comparison_em.py)
...
`view=growth_comparison`; the requested A-share listing supplies the
exchange-prefixed six-digit upstream `symbol` and Eastmoney filter. The current
wrapper emits 21 fields: `代码`, `简称`, 18 nullable growth-rate fields and the
provider's `基本每股收益增长率-3年复合排名`. The first two rows are
`行业平均` and `行业中值`, subsequent rows are ascending-ranked peers and the
requested listing is the final row. The provider freezes the JSON report,
`columns=ALL`, filter, sort, client and version parameters, records this row
ordering and validates the exact schema, listing identity, nullable/signed
growth values and positive ranks.

The normalizer emits `AKSHARE_GROWTH_COMPARISON_RAW_ONLY`; the provider-defined
growth rates and peer ranking remain raw evidence only and create no canonical
growth, valuation, market, return, governance or accounting fact. H-share
requests and any calculation, gate, pipeline, CLI or input-loader use remain
outside this slice.
...
The A-share B-share daily-history slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock/stock_zh_b_sina.py)
document `stock_zh_b_daily` as a Shanghai 900xxx or Shenzhen 200xxx B-share
series with inclusive `start_date`/`end_date` and five adjustment choices:
`''`, `qfq`, `hfq`, `qfq-factor` and `hfq-factor`. The adapter derives the
lower-prefixed symbol, preserves the exact regular fields `date`, `open`,
`high`, `low`, `close`, `volume`, `outstanding_share`, `turnover` or the exact
two-field factor response, and records Sina's full-history, outstanding-share
and optional adjustment-factor URLs plus the wrapper decoder and transforms.
Factor responses are retained as full descending histories because the
official wrapper does not apply the requested date range to those responses.

The normalizer emits `AKSHARE_B_DAILY_HISTORY_RAW_ONLY`; B-share prices have no
documented currency in this response, and its adjustment/trading-calendar
semantics are not reconciled to the canonical daily-history contract. No
canonical daily-history, return, valuation or accounting fact is created.
Shanghai and Shenzhen B-share requests and any calculation, gate, pipeline,
CLI or input-loader use remain outside this slice.

The H-share Sina daily-history slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock/stock_hk_sina.py)
document `stock_hk_daily` as a symbol-scoped full-history series with `adjust`
choices `''`, `qfq`, `hfq`, `qfq-factor` and `hfq-factor`. The adapter exposes
it under `MARKET_HISTORY` with explicit `view=hk_daily`, accepts H-share
listings only, passes the six-digit listing symbol without date arguments and
preserves the exact regular six-field `date`, `open`, `high`, `low`, `close`,
`volume` order or the exact `date`, `qfq_factor` / `date`, `hfq_factor`,
`cash` factor order. Regular dates are strictly ascending; factor dates are
strictly descending. Replay metadata records the Sina K-line/factor URLs,
adjustment-factor provenance, decoders, numeric/date conversions and
undocumented numeric units.

The normalizer emits `AKSHARE_HK_DAILY_HISTORY_RAW_ONLY`; provider-owned H-share
prices, adjustment/factor behavior, trading-calendar semantics and numeric
units are not reconciled to the canonical daily-history contract. No canonical
fact is created. A-share, date-range, missing/extra-parameter and malformed
response requests remain outside this slice.

The Tencent A+H daily-history slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock/stock_zh_ah_tx.py)
document `stock_zh_ah_daily` as an H-code, year-partitioned daily endpoint with
`start_year`, `end_year` and `adjust` choices `''`, `qfq` and `hfq`. The adapter
exposes it under `MARKET_HISTORY` with explicit `view=ah_daily`, accepts H-share
listing context only, passes the documented four-argument request and preserves
the exact six-field `日期`, `开盘`, `收盘`, `最高`, `最低`, `成交量` order. Dates
must be valid, unique, strictly ascending and within the requested half-open
year range; numeric values may be finite numbers or null. Replay metadata records
the Tencent K-line URL, year partitions, adjustment mode, payload-key mapping,
decoder/projection steps and undocumented numeric units.

The normalizer emits `AKSHARE_AH_DAILY_HISTORY_RAW_ONLY`; provider-owned A+H
prices, adjustment behavior, trading-calendar semantics and numeric units are
not reconciled to the canonical daily-history contract. No canonical fact is
created. A-share requests, date arguments, malformed years, unsupported
adjustments and malformed response rows remain outside this slice.

The A-share Sina daily-history slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock/stock_zh_a_sina.py)
document `stock_zh_a_daily` as a symbol-scoped date-range series with
`start_date`, `end_date` and `adjust` choices `''`, `qfq`, `hfq`, `qfq-factor`
and `hfq-factor`. The adapter exposes it under `MARKET_HISTORY` with explicit
`view=sina_a_daily`, accepts A-share listings only, passes the lower-prefixed
symbol and normalized inclusive bounds, and preserves the exact regular
nine-field `date`, `open`, `high`, `low`, `close`, `volume`, `amount`,
`outstanding_share`, `turnover` order or the exact two-field factor order.
Regular dates are strictly ascending and within the requested range; factor
dates are strictly descending and retain full-history scope. Replay metadata
records Sina's K-line, amount and optional factor URLs, decoder/transformation
steps, derived outstanding-share/turnover operations, exact field order, row
identity order and conservative undocumented numeric units.

The normalizer emits `AKSHARE_SINA_A_DAILY_HISTORY_RAW_ONLY`; provider-owned
...
daily-history contract. No canonical fact is created. The existing no-view
A-share history fallback and any calculation, gate, pipeline, CLI or
input-loader use remain outside this slice.

The Eastmoney A-share historical slice is the Phase 3.75 standard-history
contract. The current [AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hist_em.py)
define `stock_zh_a_hist` with a six-digit `symbol`, `period` values `daily`,
`weekly` and `monthly`, inclusive `start_date`/`end_date` bounds, and `adjust`
values `''`, `qfq` and `hfq`. The adapter selects that endpoint only with
`MARKET_HISTORY` and explicit `view=eastmoney_a_hist`, maps the canonical
exchange prefix to Eastmoney `secid` market codes (`SH=1`, `SZ/BJ=0`), and
preserves the exact twelve-field `日期`, `股票代码`, OHLCV, amount, amplitude,
change-percent, change-amount and turnover order.

Provider validation binds the requested symbol, period, adjustment and
inclusive date range to every response; rows must have matching code, exact
fields/order, unique strictly ascending dates and finite numeric values or
null. Replay metadata records the fixed/dynamic Eastmoney JSON parameters,
period/adjustment codes, wrapper source-column mapping and documented
lots/CNY/percent units. The existing no-view `stock_zh_a_hist`/`stock_zh_a_daily`
fallback remains unchanged. Because this is the standard symbol-scoped history
path, the normalizer maps existing historical OHLCV/turnover/change-percent
...
realtime-quote universe. The adapter exposes this documented endpoint under
`MARKET_QUOTE` with explicit `view=hk_spot_sina`, accepts only H-share listings,
and validates the exact sixteen-field wrapper order: `日期时间`, `代码`,
`中文名称`, `英文名称`, `交易类型`, `最新价`, `涨跌额`, `涨跌幅`, `昨收`, `今开`,
`最高`, `最低`, `成交量`, `成交额`, `买一` and `卖一`. It validates every
full-universe row before provider-side filtering to the requested five-digit
code, including valid observation timestamps, ascending unique codes and
finite numeric/null values.

Replay metadata records the Sina source page, JSON endpoint, fixed page size
60, pages 1–99 until an empty response, `node=qbgg_hk`, source-column mapping
and dropped columns, numeric conversions, row observation-time order and the
selected row order. Numeric units are not documented by the endpoint, so no
unit is inferred. The existing no-view H-share quote compatibility fallback
remains unchanged.

The normalizer emits `AKSHARE_HK_SINA_SPOT_QUOTE_RAW_ONLY` and creates no
canonical current-price, return, valuation or accounting fact: this delayed
provider snapshot and its quote context are retained as raw evidence only.
Focused tests cover H-share-only routing, exact schema/order, timestamp/text/
numeric and unrequested-row validation, empty selection, raw-only
...
explicit `view=hk_spot_em`, accepts only H-share listings, and validates the
exact twelve-field wrapper order: `序号`, `代码`, `名称`, `最新价`, `涨跌额`,
`涨跌幅`, `今开`, `最高`, `最低`, `昨收`, `成交量` and `成交额`. It validates
every full-universe row, including the wrapper's positive ascending sequence
reset, unique five-digit code, required text and finite numeric/null values,
before provider-side filtering to the requested listing.

Replay metadata records the `72.push2.eastmoney.com` JSON URL, the fixed
Eastmoney query, provider-driven `pn` pagination with page size 100, the
upstream `f3` descending sort and wrapper sequence reset, source-column
projection/dropped positions, numeric conversions, documented HKD/share,
percent, shares and HKD units, and full/selected identity order. The existing
no-view H-share quote compatibility fallback remains unchanged.

The normalizer emits `AKSHARE_HK_SPOT_EM_QUOTE_RAW_ONLY` and creates no
canonical current-price, return, valuation or accounting fact: this delayed
current-day snapshot has no stable observation timestamp and its provider-owned
quote values remain raw evidence only. Focused tests cover H-share-only routing,
exact schema/order, adversarial response validation including invalid
...
The documented `stock_hk_hot_rank_detail_realtime_em` endpoint is a separate
symbol-scoped H-share market-activity slice selected with explicit
`view=hk_hot_rank_detail_realtime`. The official wrapper posts the fixed
`appId`, `globalId` and `marketType=000003` values plus the dynamic
`srcSecurityCode=HK|<five-digit-code>` to Eastmoney's
`getCurrentHkUsList` JSON endpoint, then exposes the exact `时间` and `排名`
fields. The adapter validates every row's field order, strict timestamp
format/order and positive integer rank before retaining the provider response.

Replay metadata records the documented page URI, H-share listing scope, source
field mapping, upstream POST parameters, fixed/dynamic security identity,
decoder/rename steps, row counts and observation-time bounds. The normalizer
emits `AKSHARE_HK_HOT_RANK_DETAIL_REALTIME_RAW_ONLY`; this intraday popularity
rank series remains raw evidence only and creates no canonical market, return,
valuation, governance or accounting fact. Focused tests cover H-share-only
routing, unsupported parameters, exact-schema adversarial responses,
...
The next documented A-share market-activity inventory item is
`stock_hot_keyword_em`, selected with explicit `view=hot_keyword`. The
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock/stock_hot_rank_em.py)
document a symbol-scoped Eastmoney request that posts fixed `appId` and
`globalId` values plus the market-prefixed `srcSecurityCode` to
`getHotStockRankList`. The wrapper extracts `data`, drops its `flag` field and
renames the exact five-field response to `时间`, `股票代码`, `概念名称`,
`概念代码`, `热度`.

The adapter accepts A-share listings only and validates exact field order,
timestamp format and single-snapshot consistency, market-prefixed requested
listing identity, non-empty concept labels/codes, unique concept codes and
non-negative integer heat values. Replay metadata records the documented page
URI, upstream POST URL/parameters, fixed/dynamic security identity, wrapper
source order/drop/rename mapping, conservative unit treatment, concept-code
order, observation time and row counts. The response is raw-only and the
normalizer emits `AKSHARE_HOT_KEYWORD_RAW_ONLY`; provider-defined concept
labels, codes and heat values do not become canonical market, return,
valuation, governance or accounting facts. Focused tests cover A-share-only
routing, unsupported parameters, exact-schema adversarial responses, empty
...
The current [AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock/stock_hot_rank_em.py)
document `stock_hot_rank_detail_realtime_em` as a symbol-scoped recent
intraday rank series. The adapter exposes it under `MARKET_ACTIVITY` only with
explicit `view=hot_rank_detail_realtime`, accepts A-share listings only and
passes the market-prefixed listing code to the wrapper's Eastmoney JSON POST.

The wrapper contract is frozen as fixed `appId=appId01`, the documented
`globalId`, empty `marketType`, dynamic `srcSecurityCode` and the
`getCurrentList` URL. It preserves the exact two-field output order `时间`,
`排名`, validates timestamps in strict ascending order and positive integer
ranks, and accepts an empty symbol-scoped snapshot. Replay metadata records
the fixed/dynamic upstream parameters, source field order, positional mapping,
observation-time bounds and row counts.
...
Focused tests cover A-share-only routing, unsupported parameters, exact-schema
...
document `stock_zh_kcb_daily` as a single-symbol full-history series with
`adjust` choices `''`, `qfq`, `hfq`, `qfq-factor` and `hfq-factor`. The adapter
exposes it under `MARKET_HISTORY` with explicit `view=kcb_daily`, supports only
Shanghai 688xxx/689xxx STAR Market listings, and preserves the exact ten-field
regular output `date`, `open`, `high`, `low`, `close`, `volume`, `after_volume`,
`after_amount`, `outstanding_share` and `turnover`, or the exact two-field
factor output. It records Sina's JSONP K-line, outstanding-share and optional
factor URLs, the full-history scope, adjustment mode, decoder/transformation
steps and conservative field-unit metadata.

The normalizer emits `AKSHARE_KCB_DAILY_HISTORY_RAW_ONLY`; provider-derived
outstanding-share, after-hours, turnover and adjustment values are not
reconciled to the canonical daily-history contract or filing-backed market
facts. No canonical fact is created. Shenzhen/H-share requests, date-range
parameters and any calculation, gate, pipeline, CLI or input-loader use remain
outside this slice.

The Sina mainland-index daily-history slice is also acquisition-only. The
current [AKShare index-data documentation](https://akshare.akfamily.xyz/data/index/index.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/index/index_stock_zh.py)
document `stock_zh_index_daily` as a single-symbol full-history endpoint whose
example target is the Shenzhen index `sz399552`; the implementation requests
Sina's encrypted JavaScript K-line response with fixed `d=2020_2_4` and decodes
it with `hk_js_decode` and `py_mini_racer`. The adapter exposes it under
`MARKET_HISTORY` with explicit `view=index_daily`, accepts Shanghai 000xxx or
Shenzhen 399xxx index-shaped IDs, preserves the exact six-field `date`, `open`,
`high`, `low`, `close`, `volume` order and records the lower-prefixed symbol,
full-history scope, source URL, fixed/dynamic parameters, decoder steps and
undocumented numeric units in replay metadata.

The normalizer emits `AKSHARE_INDEX_DAILY_HISTORY_RAW_ONLY`; index-level OHLCV
does not establish listing/entity accounting scope or the canonical daily-
history, return, valuation or accounting inputs. No canonical fact is created.
Stock-listing, B-share, H-share, Beijing and date-range requests remain outside
this slice.

The Sina Hong Kong-index daily-history slice is also acquisition-only. The
current [AKShare index-data documentation](https://akshare.akfamily.xyz/data/index/index.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/index/index_stock_hk.py)
document `stock_hk_index_daily_sina` as a symbol-scoped H-index history
endpoint, defaulting to `CES100`. The adapter exposes it under
`MARKET_HISTORY` with explicit `view=hk_index_daily_sina` and
`index_symbol`, while retaining an H-share listing as context only. It calls
Sina's encrypted `klc2_kl.js` response with fixed `d=2023_5_01`, decodes it
with `hk_js_decode` and `py_mini_racer`, and preserves the exact six-field
`date`, `open`, `close`, `high`, `low`, `volume` order, strict ascending date
identity, full-history scope and undocumented numeric units in replay metadata.

The normalizer emits `AKSHARE_HK_INDEX_DAILY_SINA_RAW_ONLY`; index-level OHLCV
is not an H-share listing history and does not establish the canonical daily-
history, return, valuation or accounting inputs. No canonical fact is created.
A-share, missing/extra-parameter, malformed-symbol and date-range requests
remain outside this slice.

The Eastmoney Hong Kong-index daily-history slice is also acquisition-only. The
current [AKShare index-data documentation](https://akshare.akfamily.xyz/data/index/index.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/index/index_stock_hk.py)
document `stock_hk_index_daily_em` as a symbol-scoped historical Hong
Kong-index endpoint, defaulting to `HSTECF2L`. The adapter exposes it under
...
response without listing filtering. It records the provider's Eastmoney
index-symbol code-map lookup (including the `HSAHP`/`100` fallback), the fixed
JSON K-line parameters, the exact five-field `date`, `open`, `high`, `low`,
`latest` order, strict ascending date identity and undocumented numeric units
in replay metadata. The documented `fqt=1` request is preserved as
`provider_reported_fqt_1`; the adapter does not infer a canonical adjustment
meaning from that parameter.

The normalizer emits `AKSHARE_HK_INDEX_DAILY_EM_RAW_ONLY`; provider-resolved
index OHLC is not an H-share listing history and does not establish the
canonical daily-history, return, valuation or accounting inputs. No canonical
fact is created. A-share, missing/extra-parameter, malformed-symbol and
date-range requests remain outside this slice. Tests cover explicit routing,
strict request and response validation, finite/null values, empty output,
raw-only normalization, replay metadata/payload tampering and offline cache
replay. No calculation, gate, pipeline, CLI or input-loader contract changes.

The Sina US-index daily-history slice is also acquisition-only. The current
...
document `index_us_stock_sina` as a full-history US-index endpoint with the
`.INX`, `.IXIC`, `.DJI` and `.NDX` symbol choices. The adapter exposes it under
`MARKET_HISTORY` with explicit `view=us_index_sina` and `index_symbol`, accepts
either A- or H-share listing context only as provenance, and retains the exact
seven-field `date`, `open`, `high`, `low`, `close`, `volume`, `amount` order with
strict ascending date identity and undocumented numeric units in replay
metadata. It records the encrypted-JavaScript `staticdata/us/{symbol}` request,
`zh_js_decode`/`py_mini_racer` decoding, date conversion and numeric conversion.

The normalizer emits `AKSHARE_US_INDEX_SINA_RAW_ONLY`; the global index series
has no listing/entity accounting scope and does not establish the canonical
daily-history, return, valuation or accounting inputs. No canonical fact is
created. Missing/extra-parameter, unsupported-symbol, malformed-row and
listing-context requests remain outside this slice. Tests cover explicit A/H
routing, strict request and response validation, finite/null values, empty
...
The Tencent mainland-index daily-history slice is also acquisition-only. The
current [AKShare index-data documentation](https://akshare.akfamily.xyz/data/index/index.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/index/index_stock_zh.py)
document `stock_zh_index_daily_tx` as a Shanghai 000xxx or Shenzhen 399xxx
index-level history endpoint with optional `start_date` and `end_date` in
`YYYYMMDD` form. The adapter exposes it under `MARKET_HISTORY` with explicit
`view=tencent_index_daily`, preserves the exact six-field `date`, `open`,
`close`, `high`, `low`, `amount` order, and records the inclusive range,
Tencent qfq/front-adjustment mode, `amount` in lots, yearly-fetch pagination,
earliest-date lookup, source/auxiliary URLs, fixed/dynamic parameters and
decoder/transformation steps in replay metadata.

The normalizer emits `AKSHARE_TENCENT_INDEX_DAILY_HISTORY_RAW_ONLY`; its
always-front-adjusted index OHLC and lot-amount series do not establish
listing/entity accounting scope or the canonical daily-history, return,
valuation or accounting inputs. No canonical fact is created. Stock-listing,
B-share, H-share, Beijing, unsupported-parameter and reversed-range requests
remain outside this slice.

The Eastmoney mainland-index daily-history slice is also acquisition-only. The
current [AKShare index-data documentation](https://akshare.akfamily.xyz/data/index/index.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/index/index_stock_zh.py)
document `stock_zh_index_daily_em` as an Eastmoney index history endpoint with
optional `start_date` and `end_date` bounds. The adapter exposes it under
`MARKET_HISTORY` with explicit `view=index_daily_em`, accepts Shanghai 000xxx,
Shenzhen 399xxx or Beijing 899xxx index-shaped IDs, maps the lower-prefixed
symbol to the Eastmoney `secid`, and preserves the exact seven-field
`date`, `open`, `close`, `high`, `low`, `volume`, `amount` order. It records
`fqt=0` unadjusted mode, the inclusive range, source URL, fixed/dynamic
parameters, response JSON decoder, dropped internal column and wrapper
transformations in replay metadata.

The normalizer emits `AKSHARE_INDEX_DAILY_EM_RAW_ONLY`; unadjusted index
OHLCV/amount does not establish listing/entity accounting scope or the canonical
daily-history, return, valuation or accounting inputs. No canonical fact is
created. Stock-listing, H-share, unsupported index-shaped IDs, unsupported
parameters and reversed ranges remain outside this slice.

The generic Eastmoney mainland-index history slice is also acquisition-only. The
current [AKShare index-data documentation](https://akshare.akfamily.xyz/data/index/index.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/index/index_zh_em.py)
document `index_zh_a_hist` as an index-code endpoint with `daily`, `weekly` and
`monthly` periods, default dates `19700101` through `22220101`, and no market
prefix in the `symbol` argument. The adapter exposes it under `MARKET_HISTORY`
with explicit `view=index_zh_a_hist`, derives the raw index code from the
supported Shanghai 000xxx, Shenzhen 399xxx or Beijing 899xxx context, preserves
the Eastmoney index-code-map and market-fallback resolution, and records the
unadjusted period-specific `klt`, full-history fetch and inclusive wrapper date
filter in replay metadata. It retains the exact eleven-field order `日期`,
`开盘`, `收盘`, `最高`, `最低`, `成交量`, `成交额`, `振幅`, `涨跌幅`, `涨跌额`,
`换手率`.

The normalizer emits `AKSHARE_INDEX_ZH_A_HIST_RAW_ONLY`; unadjusted multi-period
index OHLCV, turnover and provider-derived change fields do not establish
listing/entity accounting scope or the canonical daily-history, return,
valuation or accounting inputs. No canonical fact is created. Stock-listing,
H-share, unsupported index-shaped IDs, unsupported parameters and reversed
ranges remain outside this slice.

The Eastmoney index minute-history slice is also acquisition-only. The current
[AKShare index-data documentation](https://akshare.akfamily.xyz/data/index/index.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/index/index_zh_em.py)
document `index_zh_a_hist_min_em` as a minute-history endpoint with periods `1`,
`5`, `15`, `30` and `60`, default datetimes `1979-09-01 09:32:00` through
`2222-01-01 09:32:00`, and a raw index-code `symbol` without a market prefix.
The adapter exposes it under `MARKET_HISTORY` with explicit
`view=index_zh_a_hist_min_em`, derives the code from supported Shanghai 000xxx,
Shenzhen 399xxx or Beijing 899xxx context, preserves the Eastmoney index-code
map and market-fallback resolution, and records the period-specific trends or
K-line endpoint, inclusive datetime filtering and adjustment mode in replay
metadata. One-minute rows retain the exact eight-field `时间`, `开盘`, `收盘`,
`最高`, `最低`, `成交量`, `成交额`, `均价` order; other periods retain the
documented eleven-field `时间`, `开盘`, `收盘`, `最高`, `最低`, `涨跌幅`,
`涨跌额`, `成交量`, `成交额`, `振幅`, `换手率` order.

The normalizer emits `AKSHARE_INDEX_ZH_A_HIST_MIN_EM_RAW_ONLY`; recent index
intraday bars, provider adjustment mode and limited history do not establish
listing/entity accounting scope or the canonical daily-history, return,
valuation or accounting inputs. No canonical fact is created. Stock-listing,
H-share, unsupported index-shaped IDs, unsupported parameters and reversed
ranges remain outside this slice.

The A-share B-share minute-history slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock/stock_zh_b_sina.py)
document `stock_zh_b_minute` as a Shanghai 900xxx or Shenzhen 200xxx B-share
minute window with `period` values `1`, `5`, `15`, `30` or `60` and adjustment
modes `''`, `qfq` or `hfq`. The adapter derives the market-prefixed symbol,
freezes the JSONP K-line request with `scale` and `datalen=1970`, preserves the
exact six-field `day`, `open`, `high`, `low`, `close`, `volume` order, and
records the adjustment transform and recent-trading-day scope in replay
metadata.

The normalizer emits `AKSHARE_B_MINUTE_HISTORY_RAW_ONLY`; the recent minute
window has no documented price/volume units and does not establish the
canonical daily-history, currency or valuation inputs. No canonical fact is
created. Shanghai and Shenzhen B-share requests and any calculation, gate,
pipeline, CLI or input-loader use remain outside this slice.

The Sina next-new-stock slice is also acquisition-only. The current [AKShare
stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock/stock_zh_a_special.py)
document `stock_zh_a_new` as a no-argument Shanghai/Shenzhen next-new-stock
universe that is available only for the latest trading day. The adapter
exposes it under `MARKET_ACTIVITY` with explicit `view=sina_new_stock`, calls
the Sina `Market_Center.getHQNodeStockCount` page-count request with
`node=new_stock`, then the JSON `Market_Center.getHQNodeData` pages with size
80, ascending `symbol` order and the exact `new_stock` filter. It preserves
the exact ten-field `symbol`, `code`, `name`, `open`, `high`, `low`, `volume`,
`amount`, `mktcap`, `turnoverratio` order and complete-universe identity
metadata before selecting the requested Shanghai or Shenzhen listing.

The latest-trading-day quote universe has no stable observation date or
filing-backed valuation/unit basis. The normalizer emits
`AKSHARE_SINA_NEW_STOCK_RAW_ONLY`; no canonical market, return, governance,
valuation or accounting fact is created. Tests cover exact routing, full
universe field/order/identity/type validation, invalid unrequested rows,
empty selection, raw-only normalization, replay metadata tampering and
offline cache replay. No calculation, gate, pipeline, CLI or input-loader
contract changes.

The Eastmoney company-dynamics calendar slice is also acquisition-only. The
current [AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock/stock_gsrl_em.py)
document `stock_gsrl_gsdt_em` as a requested-trading-day full-universe response
with `date=YYYYMMDD`. The adapter exposes it under `MARKET_ACTIVITY` with
explicit `view=company_dynamics`, calls the `RPT_ORGOP_ALL` JSON report with
the exact `TRADE_DATE` filter, ascending `SECURITY_CODE` sort and 5000-row page,
and preserves the exact six-field `序号`, `代码`, `简称`, `事件类型`, `具体事项`,
`交易日` wrapper order after dropping the internal `SECUCODE` field. It
validates every date-bound row before selecting the requested A-share listing
and records the upstream filter, wrapper field order and complete/selected row
counts in replay metadata.

The normalizer emits `AKSHARE_COMPANY_DYNAMICS_RAW_ONLY`; event labels and
descriptions do not establish filing contents, accounting periods, governance
conclusions or a canonical market fact. No canonical fact is created. Tests
cover date/view routing, exact schema and field order, complete-universe
validation, duplicate listing events, empty selection, raw-only normalization,
replay metadata tampering and offline cache replay. No calculation, gate,
pipeline, CLI or input-loader contract changes.

The Tonghuashun new-stock-first-day slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_board_industry_ths.py)
document `stock_xgsr_ths` as a no-argument historical new-stock listing-day
universe. The adapter exposes it under `MARKET_ACTIVITY` with explicit
`view=new_stock_first_day`, records the `field=SSRQ` descending sort, the
provider-reported HTML page count and the `ths.js` `v`/`hexin-v` authentication
contract, and preserves the exact twelve-field wrapper order after renaming
`发行价(元)` to `发行价`, converting numeric values and expressing `首日涨跌幅`
as a ratio. It validates the complete source universe before selecting the
requested A-share listing and records source identity/date order plus the
selected positions for replay.

The normalizer emits `AKSHARE_NEW_STOCK_FIRST_DAY_RAW_ONLY`; historical prices,
returns and issue-status labels do not establish a canonical listing, return,
valuation or accounting fact. No canonical fact is created. Tests cover
explicit view routing, exact field/order and HTML-source metadata,
...
The Tonghuashun IPO-benefit slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_board_industry_ths.py)
document `stock_ipo_benefit_ths` as a no-argument, recently updated IPO-benefit
universe. The adapter exposes it under `MARKET_ACTIVITY` with explicit
`view=ipo_benefit`, records the weekly snapshot, `field=invest` descending
sort, provider-reported HTML page count and `ths.js` `v`/`hexin-v`
authentication contract, and preserves the exact ten-field wrapper order. It
validates the complete source universe before selecting the requested A-share
listing and records source identity order plus selected positions for replay.

The normalizer emits `AKSHARE_IPO_BENEFIT_RAW_ONLY`; provider-defined investee
counts, investment amounts and market-value ratios do not establish issuer
revenue, cash flow, ownership, valuation or a canonical accounting fact. No
canonical fact is created. Tests cover explicit view routing, exact field/order
and HTML-source metadata, complete-universe validation, empty selection,
raw-only normalization, replay metadata tampering and offline cache replay. No
calculation, gate, pipeline, CLI or input-loader contract changes.

The Eastmoney two-net-and-delisted-stock slice is also acquisition-only. The
current [AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock/stock_zh_a_special.py)
document `stock_zh_a_stop_em` as a no-argument current-trading-day universe. The
adapter exposes it under `MARKET_ACTIVITY` with explicit `view=stop_stock`,
freezes the `40.push2.eastmoney.com` JSON query (`fs=m:0 s:3`, page size 100),
records the provider's `f3` descending sort and all-page pagination, and
preserves the exact seventeen-field wrapper order plus its 33-column mapping.
It validates the complete source universe before selecting the requested A-share
listing and records source identity order plus selected positions for replay.

The normalizer emits `AKSHARE_STOP_STOCK_RAW_ONLY`; current quote values and
provider-defined two-net/delisted membership do not establish a dated listing,
canonical price, valuation or accounting fact. No canonical fact is created.
Tests cover explicit view routing, exact schema and field order, JSON-source
metadata, complete-universe validation, empty selection, raw-only normalization,
replay metadata tampering and offline cache replay. No calculation, gate,
pipeline, CLI or input-loader contract changes.

The Sina STAR Market quote slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock/stock_zh_kcb_sina.py)
document `stock_zh_kcb_spot` as a no-argument full 科创板 realtime-quote
universe. The adapter exposes it under `MARKET_QUOTE` with explicit
`view=kcb_sina_spot`, freezes the `node=kcb` count and paginated JSON requests
(`num=80`, ascending `symbol` order), and preserves the exact nineteen-field
wrapper order after dropping the internal second column. It retains the
provider's lower-prefixed Shanghai symbols, `时点` observation string, numeric
conversion contract and full-universe filtering metadata before selecting the
requested 688xxx/689xxx listing.

The normalizer emits `AKSHARE_KCB_SINA_SPOT_QUOTE_RAW_ONLY`; the realtime
quote, valuation ratios and provider observation time do not establish the
canonical current-price input. No canonical fact is created. Tests cover
explicit view routing, exact schema and field order, JSON-source pagination
metadata, complete-universe validation, invalid unrequested rows, empty
selection, raw-only normalization, replay metadata tampering and offline cache
replay. No calculation, gate, pipeline, CLI or input-loader contract changes.
...
The H-share Stock Connect constituent quote slice is also acquisition-only. The
current [AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hsgt_em.py)
document `stock_hk_ggt_components_em` as a no-argument, paginated H-share
Stock Connect constituent universe request. The adapter exposes it under
`MARKET_QUOTE` with explicit `view=hk_ggt_components`, freezes the
`https://33.push2.eastmoney.com/api/qt/clist/get` request with `pz=100`,
`fid=f12`, `fs=b:DLMK0146,b:DLMK0144` and the official field list, validates
every row in the complete five-digit H-share response before filtering to the
requested listing, and records the source page, pagination, HKD/share, percent,
shares and HKD units plus full/selected row counts in replay metadata. The
wrapper returns exactly 12 fields: `序号`, `代码`, `名称`, `最新价`, `涨跌额`, `涨跌幅`,
`今开`, `最高`, `最低`, `昨收`, `成交量` and `成交额`.

The documented quote is a 15-minute-delayed current-day snapshot without a
stable observation timestamp. The normalizer emits
`AKSHARE_HK_GGT_COMPONENTS_QUOTE_RAW_ONLY`; the selected rows remain raw
evidence and create no canonical current-price, return, valuation or accounting
fact. A-share requests and any calculation, gate, pipeline, CLI or input-loader
use remain outside this slice.

The HSGT minute-fund-flow slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hsgt_min_em.py)
document `stock_hsgt_fund_min_em` as a market-wide northbound/southbound
intraday flow view with `symbol=北向资金` or `symbol=南向资金`; the
documentation states that the source stopped providing data from 2024-05-13.
The adapter exposes it under `CAPITAL_FLOW` with explicit
`view=hsgt_fund_min`, accepts northbound only with A-share context and
southbound only with H-share context, and freezes the exact JSON endpoint
`https://push2.eastmoney.com/api/qt/kamtbs.rtmin/get`, fixed `fields1`,
`fields2`, `ut` and `_` parameters, wrapper field order and `万元` flow units.
It validates one observation date, strict time order and finite numeric/null
values while retaining the complete raw response as evidence; there is no
listing-row filtering because the upstream response is a market-wide flow
snapshot. The normalizer emits `AKSHARE_HSGT_FUND_MIN_RAW_ONLY`: these amounts
are not issuer cash flow, a listing-specific liquidity measure or a canonical
return/valuation input. A-share/H-share requests and any calculation, gate,
pipeline, CLI or input-loader use remain outside this slice.

The HSGT board-rank slice is also acquisition-only. The current [AKShare
stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hsgt_em.py)
document `stock_hsgt_board_rank_em` as a market-wide northbound board ranking
with three board selectors and seven interval selectors. The adapter exposes it
under `MARKET_ACTIVITY` with explicit `view=hsgt_board_rank`, maps the selectors
to the official `BOARD_TYPE`/`INTERVAL_TYPE` codes, derives the report date from
the documented Eastmoney wrapper page's `bkph_date`, and freezes the
`RPT_MUTUAL_BOARD_HOLDRANK_WEB` JSON request, sort, page-size, quote-column and
filter metadata. It validates the exact 17-field source order, ascending
sequence, unique board names, constant report date, finite numeric/null values
and the four text-valued largest-increase/decrease fields. The response is
market-wide and not listing-scoped, so the normalizer emits
`AKSHARE_HSGT_BOARD_RANK_RAW_ONLY` and creates no canonical issuer cash-flow,
shareholder-return, governance, valuation or market fact. H-share requests and
any calculation, gate, pipeline, CLI or input-loader use remain outside this
slice.

The HSGT individual-ranking slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hsgt_em.py)
document `stock_hsgt_hold_stock_em` as a full A-share HSGT ranking universe
with `market=北向`, `沪股通` or `深股通` and `indicator=今日排行`, `3日排行`,
`5日排行`, `10日排行`, `月排行`, `季排行` or `年排行`. The adapter maps the
markets to the official `MUTUAL_TYPE` values `001` and `003` where applicable,
maps the periods to `INTERVAL_TYPE` values `1`, `3`, `5`, `10`, `M`, `Q` and
`Y`, derives the report date from `div.title span`, and freezes the exact
`RPT_MUTUAL_STOCK_NORTHSTA` JSON request, descending `ADD_MARKET_CAP` sort,
`50000` page size, all-page pagination and date/market/interval filter. It
preserves the official dynamic 16-field order: the five `增持估计` fields are
renamed with the selected period, provider numeric values are retained with
their documented 万股/万元/百分比/千分比 units, and the undocumented close
price remains explicitly untyped. The complete ranked response is validated
before any requested-listing filtering, with full and selected rank/code order
bound into replay metadata.

The normalizer emits `AKSHARE_HSGT_HOLD_STOCK_RAW_ONLY`; investor-position
rankings, holdings, ratios and provider-estimated changes do not establish
beneficial control, issuer cash flow or a canonical diluted-share series.
Tests cover all market/period combinations, exact dynamic fields and values,
...
The HSGT daily stock-statistics slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hsgt_em.py)
document `stock_hsgt_stock_statistics_em` as a date-range full-universe
response with the four selectors `北向持股`, `沪股通持股`,
`深股通持股` and `南向持股`. The adapter exposes it with
explicit `view=hsgt_stock_statistics` and inclusive `start_date`/
`end_date` parameters, maps the three northbound selectors to
`RPT_MUTUAL_STOCK_NORTHSTA` and the southbound selector to
`RPT_MUTUAL_STOCK_HOLDRANKS`, and freezes the official
`INTERVAL_TYPE=1`, mutual-type/`RN=1`, date, sort, page-size and
all-page filter contract. It validates the exact 11-field wrapper output,
fixed-width A/H codes, descending date order, unique date/code identities,
finite numeric/null values and full-universe row order before selecting the
requested listing. CNY/HKD price, quantity, market-value, percentage and
market-value-change units are retained per symbol in replay metadata.
...
beneficial control, issuer cash flow or a canonical diluted-share series.
Tests cover all four selectors, exact filters/reports and units,
request/context rejection, full-universe schema/date/type boundaries, empty
...
The HSGT Shanghai-to-Hong Kong quote slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock/stock_hsgt_em.py)
document `stock_hsgt_sh_hk_spot_em` as a no-argument H-share Shanghai Stock
Connect snapshot. The adapter exposes it with explicit `view=hk_sh_spot`, calls
the Eastmoney `push2.eastmoney.com/api/qt/clist/get` JSON universe with fixed
`b:DLMK0144` filtering, and preserves the exact 12-field wrapper order. It
records the `f12`/`f14`/`f2`/`f4`/`f3`/`f17`/`f15`/`f16`/`f18`/`f5`/`f6`
mapping, divides prices and change amounts by `1000`, percentages by `100` and
volume/turnover by `100000000`, and validates the complete five-digit-code
universe before provider-side selection. Replay metadata retains full and
selected code order, the source filter, field transforms and unit context.

The current-trading-day quote is delayed by 15 minutes and has no stable
observation timestamp. The normalizer emits
`AKSHARE_HK_SH_SPOT_QUOTE_RAW_ONLY`; no canonical current-price or other market
fact is created. Tests cover request/context rejection, complete-universe
schema/rank/code/type boundaries, invalid unrequested rows, empty selection,
raw-only normalization, replay metadata tampering and offline cache replay. No
calculation, gate, pipeline, CLI or input-loader contract changes.

The HSGT historical-flow slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hsgt_em.py)
document `stock_hsgt_hist_em` as a paginated market-wide history
with six symbols: `北向资金`, `沪股通`,
`深股通`, `南向资金`, `港股通沪` and
`港股通深`. The adapter exposes it under `CAPITAL_FLOW`
with explicit `view=hsgt_hist`, maps each symbol to the official
`MUTUAL_TYPE` code, and freezes the
`RPT_MUTUAL_DEAL_HISTORY` JSON request, descending upstream
`TRADE_DATE` order, page size `1000` and all-page
pagination. It preserves the symbol-specific 13-field output including daily
flow, cumulative flow, holdings value, leading-stock and index context, validates
the complete response in strict ascending date order, and records the exact
filter, fields, transformations and documented units for replay.

The rows have no requested-listing identity; A/H listing context remains request
metadata only with `row_filtering=none` and
`entity_rows_selected=false`. The normalizer emits
`AKSHARE_HSGT_HIST_RAW_ONLY` and creates no canonical issuer
cash-flow, listing-specific liquidity, return or valuation fact. Tests cover all
six symbols, exact report/filter metadata, context and parameter rejection,
complete-response schema/date/numeric/text boundaries, raw-only normalization,
replay metadata tampering and offline cache replay. No calculation, gate,
pipeline, CLI or input-loader contract changes.

The HSGT individual-detail slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hsgt_em.py)
document `stock_hsgt_individual_detail_em` as an A-share symbol-scoped
institution-holding detail response with `symbol`, `start_date` and
`end_date`. The wrapper calls the Eastmoney `RPT_MUTUAL_HOLD_DET` JSON report,
first using `MARKET_CODE="003"` and falling back to `MARKET_CODE="001"` when
the first result is absent, with descending `HOLD_DATE`, page size `500` and
provider-driven all-page pagination. The adapter exposes it only with explicit
`view=hsgt_individual_detail`, requires a six-digit A-share listing context and
inclusive `YYYYMMDD` bounds, validates the exact ten-field output, grouped
non-increasing dates, institution identities and finite numeric/null values,
and records the 17-column wrapper mapping plus both market-code filters.

The rows are listing-scoped institution holding evidence, not a complete
beneficial-ownership or company share-capital series. The normalizer emits
...
empty listing selections, raw-only normalization, replay metadata tampering
...
The Shanghai A-share quote slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hist_em.py)
document `stock_sh_a_spot_em` as a no-argument Shanghai A-share real-time
snapshot. The adapter exposes it with explicit `view=sh_a_spot`, calls the
Eastmoney `push2.eastmoney.com/api/qt/clist/get` JSON universe with the fixed
`m:1 t:2,m:1 t:23` filter, provider-driven all-page pagination, `f3`
descending sort and the exact 23-field wrapper order. It preserves the
wrapper's 33-column source mapping, documented price/volume/turnover/percentage
units and complete-universe code order before selecting the requested Shanghai
listing.

The current-day quote has no stable observation timestamp. The normalizer emits
`AKSHARE_SH_A_SPOT_QUOTE_RAW_ONLY`; no canonical current-price or other market
fact is created. Tests cover Shanghai-only request routing, exact fields and
upstream parameters, complete-universe schema/rank/code/type boundaries,
invalid unrequested rows, empty selection, raw-only normalization, replay
metadata tampering and offline cache replay. No calculation, gate, pipeline,
CLI or input-loader contract changes.

The Shenzhen A-share quote slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hist_em.py)
document `stock_sz_a_spot_em` as a no-argument Shenzhen A-share real-time
snapshot. The adapter exposes it with explicit `view=sz_a_spot`, calls the
Eastmoney `push2.eastmoney.com/api/qt/clist/get` JSON universe with the fixed
`m:0 t:6,m:0 t:80` filter, provider-driven all-page pagination, `f3`
descending sort and the exact 23-field wrapper order. It preserves the
wrapper's 33-column source mapping, documented price/volume/turnover/percentage
units and complete-universe code order before selecting the requested Shenzhen
listing.

The current-day quote has no stable observation timestamp. The normalizer emits
`AKSHARE_SZ_A_SPOT_QUOTE_RAW_ONLY`; no canonical current-price or other market
fact is created. Tests cover Shenzhen-only request routing, exact fields and
upstream parameters, complete-universe schema/rank/code/type boundaries,
invalid unrequested rows, empty selection, raw-only normalization, replay
metadata tampering and offline cache replay. No calculation, gate, pipeline,
CLI or input-loader contract changes.

The Beijing A-share quote slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hist_em.py)
document `stock_bj_a_spot_em` as a no-argument Beijing A-share real-time
snapshot. The adapter exposes it with explicit `view=bj_a_spot`, calls the
Eastmoney `push2.eastmoney.com/api/qt/clist/get` JSON universe with the fixed
`m:0 t:81 s:2048` filter, provider-driven all-page pagination, `f3`
descending sort and the exact 23-field wrapper order. It preserves the
wrapper's 33-column source mapping, documented price/volume/turnover/percentage
units and complete-universe code order before selecting the requested Beijing
listing.

The current-day quote has no stable observation timestamp. The normalizer emits
`AKSHARE_BJ_A_SPOT_QUOTE_RAW_ONLY`; no canonical current-price or other market
fact is created. Tests cover Beijing-only request routing, exact fields and
upstream parameters, complete-universe schema/rank/code/type boundaries,
invalid unrequested rows, empty selection, raw-only normalization, replay
metadata tampering and offline cache replay. No calculation, gate, pipeline,
CLI or input-loader contract changes.

The A-share new-stock quote slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hist_em.py)
document `stock_new_a_spot_em` as a no-argument A-share new-stock real-time
snapshot for the `#newshares` page. The adapter exposes it with explicit
`view=new_a_spot`, calls the Eastmoney `push2.eastmoney.com/api/qt/clist/get`
JSON universe with fixed `m:0 f:8,m:1 f:8`, `fid=f26`, provider-driven
all-page pagination and `f3` descending sort, and preserves the exact
24-field wrapper order including `上市日期`. It also preserves the
wrapper's 33-column source mapping, listing-date field and bounds, documented
price/volume/turnover/percentage units and complete-universe code order before
selecting the requested A-share listing.

The current-day new-stock quote has no stable observation timestamp. The
normalizer emits `AKSHARE_NEW_A_SPOT_QUOTE_RAW_ONLY`; no canonical current-price
or other market fact is created. Tests cover A-share-only request routing,
exact fields and upstream parameters, complete-universe schema/rank/code/date/
type boundaries, invalid unrequested rows, empty selection, raw-only
normalization, replay metadata tampering and offline cache replay. No
calculation, gate, pipeline, CLI or input-loader contract changes.

The Growth Enterprise Market quote slice is also acquisition-only. The current
[AKShare stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hist_em.py)
document `stock_cy_a_spot_em` as a no-argument Shenzhen Growth Enterprise Market
real-time snapshot for the `#gem_board` page. The adapter exposes it with
explicit `view=cy_a_spot`, calls `7.push2.eastmoney.com/api/qt/clist/get` with
fixed `m:0 t:80`, `fid=f12`, `wbp2u=|0|0|0|web`, provider-driven all-page
pagination and `f3` descending sort, and preserves the exact 23-field wrapper
order. It also preserves the wrapper's 33-column source mapping, documented
price/volume/turnover/percentage units and complete-universe code order before
selecting the requested Growth Enterprise Market listing.

The current-day quote has no stable observation timestamp. The normalizer emits
`AKSHARE_CY_A_SPOT_QUOTE_RAW_ONLY`; no canonical current-price or other market
fact is created. Tests cover Growth Enterprise Market-only request routing,
exact fields and upstream parameters, complete-universe schema/rank/code/type
boundaries, invalid unrequested rows, empty selection, raw-only normalization,
replay metadata tampering and offline cache replay. No calculation, gate,
pipeline, CLI or input-loader contract changes.

The STAR Market quote slice is also acquisition-only. The current [AKShare
stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hist_em.py)
document `stock_kc_a_spot_em` as a no-argument Shanghai STAR Market real-time
snapshot for the `#kcb_board` page. The adapter exposes it with explicit
`view=kc_a_spot`, calls `7.push2.eastmoney.com/api/qt/clist/get` with fixed
`m:1 t:23`, `fid=f12`, `wbp2u=|0|0|0|web`, provider-driven all-page
pagination and `f3` descending sort, and preserves the exact 23-field wrapper
order. It also preserves the wrapper's 33-column source mapping, documented
price/volume/turnover/percentage units and complete-universe code order before
selecting the requested STAR Market listing.

The current-day quote has no stable observation timestamp. The normalizer emits
`AKSHARE_KC_A_SPOT_QUOTE_RAW_ONLY`; no canonical current-price or other market
fact is created. Tests cover STAR Market-only request routing, exact fields and
upstream parameters, complete-universe schema/rank/code/type boundaries,
invalid unrequested rows, empty selection, raw-only normalization, replay
metadata tampering and offline cache replay. No calculation, gate, pipeline,
CLI or input-loader contract changes.

The Eastmoney mainland-index spot slice is also acquisition-only. The current
[AKShare index-data documentation](https://akshare.akfamily.xyz/data/index/index.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/index/index_stock_zh.py)
document `stock_zh_index_spot_em` as a real-time index-universe endpoint with
the category selectors `沪深重要指数`, `上证系列指数`, `深证系列指数`, `指数成份`
and `中证系列指数`. The adapter exposes it under `MARKET_QUOTE` with explicit
`view=index_spot` and a required `symbol` selector, preserves the complete
provider response without listing filtering, and records whether the wrapper
used the important-index or general Eastmoney JSON endpoint.

The wrapper returns the exact fourteen fields `序号`, `代码`, `名称`, `最新价`,
`涨跌幅`, `涨跌额`, `成交量`, `成交额`, `振幅`, `最高`, `最低`, `今开`, `昨收`
and `量比`. Only the documented percentage units for `涨跌幅` and `振幅` are
asserted; other numeric units remain provider-reported and undocumented. The
current-day index universe has no stable listing observation timestamp and no
issuer accounting scope. The normalizer emits
`AKSHARE_INDEX_SPOT_RAW_ONLY`; no canonical current-price or other market
fact is created. Tests cover all documented selectors, exact schema/order and
numeric/rank boundaries, unsupported listing/parameter requests, empty output,
raw-only normalization, replay metadata tampering and offline cache replay. No
calculation, gate, pipeline, CLI or input-loader contract changes.

The Sina mainland-index spot slice is also acquisition-only. The current
[AKShare index-data documentation](https://akshare.akfamily.xyz/data/index/index.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/index/index_stock_zh.py)
document `stock_zh_index_spot_sina` as a no-argument real-time universe for the
`hs_s` node. The adapter exposes it under `MARKET_QUOTE` with explicit
`view=index_spot_sina`, discovers the provider-reported page count, calls the
80-row `Market_Center.getHQNodeDataSimple` pages sorted by `symbol` ascending,
and preserves the complete response without listing filtering.

The wrapper returns the exact eleven fields `代码`, `名称`, `最新价`, `涨跌额`,
`涨跌幅`, `昨收`, `今开`, `最高`, `最低`, `成交量` and `成交额`. Only the
documented `涨跌幅` percentage, `成交量` lots and `成交额` CNY units are
asserted; other numeric fields remain undocumented. The current-day index
universe has no stable listing observation timestamp and no issuer accounting
scope. The normalizer emits `AKSHARE_INDEX_SPOT_SINA_RAW_ONLY`; no canonical
current-price or other market fact is created. Tests cover exact fields/order,
code ordering and finite/null values, unsupported listing/parameter requests,
empty output, raw-only normalization, replay metadata tampering and offline
cache replay. No calculation, gate, pipeline, CLI or input-loader contract
changes.

The Sina Hong Kong-index spot slice is also acquisition-only. The current
[AKShare index-data documentation](https://akshare.akfamily.xyz/data/index/index.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/index/index_stock_hk.py)
document `stock_hk_index_spot_sina` as a no-argument real-time universe for the
`#zs_hk` page. The adapter exposes it under `MARKET_QUOTE` with explicit
`view=hk_index_spot_sina`, requests Sina's fixed `hq.sinajs.cn` symbol list in
one quoted-text response, and preserves the complete response without listing
filtering.

The wrapper returns the exact nine fields `代码`, `名称`, `最新价`, `涨跌额`,
`涨跌幅`, `昨收`, `今开`, `最高` and `最低`. Only the documented `涨跌幅`
percentage unit is asserted; other numeric fields remain undocumented. The
current Hong Kong-index universe has no stable listing observation timestamp
and no issuer accounting scope. The normalizer emits
`AKSHARE_HK_INDEX_SPOT_SINA_RAW_ONLY`; no canonical current-price or other
market fact is created. Tests cover H-share-only routing, exact schema/order
and code/type boundaries, invalid unrequested rows, empty output, raw-only
normalization, replay metadata tampering and offline cache replay. No
calculation, gate, pipeline, CLI or input-loader contract changes.

The Eastmoney Hong Kong-index spot slice is also acquisition-only. The current
[AKShare index-data documentation](https://akshare.akfamily.xyz/data/index/index.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/index/index_stock_hk.py)
document `stock_hk_index_spot_em` as a no-argument real-time Hong Kong-index
universe. The adapter exposes it under `MARKET_QUOTE` with explicit
`view=hk_index_spot_em`, accepts H-share listing context and retains the
complete provider response without listing filtering.

The wrapper returns the exact thirteen fields `序号`, `内部编号`, `代码`, `名称`,
`最新价`, `涨跌额`, `涨跌幅`, `今开`, `最高`, `最低`, `昨收`, `成交量` and `成交额`.
The documented `涨跌幅` percentage and `成交额` HKD units are recorded; other
numeric fields remain provider-reported and undocumented. The current
Hong Kong-index universe has no stable listing observation timestamp and no
issuer accounting scope. The normalizer emits
`AKSHARE_HK_INDEX_SPOT_EM_RAW_ONLY`; no canonical current-price or other market
fact is created. Tests cover H-share-only routing, exact schema/order and
rank/code/type boundaries, empty output, raw-only normalization, replay
metadata/payload tampering and offline cache replay. No calculation, gate,
pipeline, CLI or input-loader contract changes.

The Eastmoney global-index spot slice is also acquisition-only. The current
[AKShare index-data documentation](https://akshare.akfamily.xyz/data/index/index.html)
...
provenance and preserves the complete response without listing filtering.

The wrapper returns the exact twelve fields `序号`, `代码`, `名称`, `最新价`,
`涨跌额`, `涨跌幅`, `开盘价`, `最高价`, `最低价`, `昨收价`, `振幅` and
`最新行情时间`. Only the documented `涨跌幅` and `振幅` percentage units are
asserted; other numeric fields remain provider-reported and undocumented. The
wrapper-generated rank must reset from one in source order, codes must be
unique, timestamps must be valid or null, and all numeric values must be finite
or null. The fixed Eastmoney global universe filter, single-page request,
`f3` provider ordering, positional mapping, numeric division by 100 and Unix
seconds to Asia/Shanghai timestamp conversion remain replay metadata.

The current global-index universe has no listing/entity accounting scope. The
normalizer emits `AKSHARE_GLOBAL_INDEX_SPOT_RAW_ONLY` and creates no canonical
current-price or other market fact. Tests cover A/H-context routing, strict
parameters, exact schema/rank/code/timestamp/value boundaries, empty output,
raw-only normalization, replay metadata/payload tampering and offline cache
replay. No calculation, gate, pipeline, CLI or input-loader contract changes.

The Eastmoney global-index daily-history slice is also acquisition-only. The
current [AKShare index-data documentation](https://akshare.akfamily.xyz/data/index/index.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/index/index_global_em.py)
document `index_global_hist_em` as a symbol-selected full-history endpoint.
The adapter exposes it under `MARKET_HISTORY` with explicit
`view=global_index_hist` and `index_symbol`, accepts A- or H-share listing
context only as provenance, and preserves the complete response without
listing filtering. The documented selector must be one of the names in the
local Eastmoney symbol map, including `美元指数`.

The wrapper returns the exact eight fields `日期`, `代码`, `名称`, `今开`,
`最新价`, `最高`, `最低` and `振幅`. Dates are strictly ascending without
duplicates; the mapped code must match the requested global index; the name
must be stable; and numeric values must be finite numbers or null. The replay
contract records the Eastmoney JSON `stock/kline/get` URL, dynamic `secid`,
fixed daily/full-history parameters, local symbol-map resolution, sixteen
source columns with eight dropped placeholders, positional field mapping,
date/numeric conversion and the absence of date-range filtering or auxiliary
lookup.

The global-index history has no listing/entity accounting scope. The normalizer
emits `AKSHARE_GLOBAL_INDEX_HISTORY_RAW_ONLY` and creates no canonical
daily-history, return, valuation or accounting fact. Tests cover A/H-context
routing, strict request and response validation, finite/null values, empty
output, raw-only normalization, replay metadata/payload tampering and offline
cache replay. No calculation, gate, pipeline, CLI or input-loader contract
changes.

The Sina global-index daily-history slice is also acquisition-only. The current
[AKShare index-data documentation](https://akshare.akfamily.xyz/data/index/index.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/index/index_global_sina.py)
document `index_global_hist_sina` as a name-selected recent-history endpoint.
The adapter exposes it under `MARKET_HISTORY` with explicit
`view=global_index_hist_sina` and `index_symbol`, accepts A- or H-share listing
context only as provenance, and retains the complete response without listing
filtering. The selector must be one of the documented names in the local Sina
symbol map, which is resolved to the upstream index code before the daily JSON
request.

The wrapper returns the exact six fields `date`, `open`, `high`, `low`, `close`
and `volume`. Dates are strictly ascending without duplicates; numeric values
are finite numbers or null; and responses over the documented recent 1000-row
limit are rejected. Replay metadata records the Sina daily JSON URL, mapped
`symbol`, fixed `num=10000`, source-field rename from `d/o/h/l/c/v`, date and
numeric conversion, recent-window boundary, and row identity order.

The Sina global-index history has no listing/entity accounting scope. The
normalizer emits `AKSHARE_GLOBAL_INDEX_SINA_RAW_ONLY` and creates no canonical
daily-history, return, valuation or accounting fact. Tests cover A/H-context
routing, strict request and response validation, finite/null values, the
documented row limit, empty output, raw-only normalization, replay
metadata/payload tampering and offline cache replay. No calculation, gate,
pipeline, CLI or input-loader contract changes.
...
stock-data documentation](https://akshare.akfamily.xyz/data/stock/stock.html)
and [official implementation](https://github.com/akfamily/akshare/blob/main/akshare/stock_feature/stock_hist_em.py)
document `stock_zh_b_spot_em` as a no-argument Shanghai 900xxx or Shenzhen
200xxx B-share real-time snapshot for the `#hs_b_board` page. The adapter exposes
it with explicit
`view=b_spot`, calls `28.push2.eastmoney.com/api/qt/clist/get` with fixed
`m:0 t:7,m:1 t:3`, `fid=f12`, provider-driven all-page pagination and `f3`
descending sort, and preserves the exact 23-field wrapper
order. It also preserves the wrapper's 33-column source mapping, documented
price/volume/turnover/percentage units and complete-universe code order before
selecting the requested B-share listing.

The current-day B-share quote has no stable observation timestamp. The endpoint
does not document the B-share currency. The normalizer emits
`AKSHARE_B_SPOT_QUOTE_RAW_ONLY`; no canonical current-price or other market
fact is created. Tests cover B-share-only request routing, exact fields and
upstream parameters, complete-universe schema/rank/code/type boundaries,
invalid unrequested rows, empty selection, raw-only normalization, replay
metadata tampering and offline cache replay. No calculation, gate, pipeline,
CLI or input-loader contract changes.
