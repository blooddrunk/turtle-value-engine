...
## 12. Phase 2.2–2.52 AKShare adapter
...
A-share intraday-history and five-level bid-ask raw slices. It
...
| `MARKET_HISTORY` | `stock_zh_a_hist` (daily history); `stock_zh_a_hist_min_em` (`view=intraday`, explicit datetime range/interval/adjustment; A-share only) | `stock_hk_daily` | dated daily OHLCV/turnover extension facts; intraday rows as raw structured evidence only |
...
individual-info, individual-fund-flow, intraday-history and five-level bid-ask
raw slices it emits
...
This foundation plus the Phase 2.2–2.52 slices does not include:
