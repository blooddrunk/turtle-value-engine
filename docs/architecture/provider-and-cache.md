...
## 12. Phase 2.2–2.49 AKShare adapter
...
Dragon-Tiger detail/statistics market-activity,
...
| `MARKET_ACTIVITY` | `stock_lhb_detail_em` (inclusive `start_date`/`end_date`; A-share only); `stock_lhb_stock_statistic_em` (`view=stock_statistic`, explicit `period`; A-share only) | — | A-share Dragon-Tiger detail or per-listing statistics rows filtered to the requested listing as raw structured evidence only; no issuer cash-flow, shareholder-return, governance, market or valuation fact |
...
This foundation plus the Phase 2.2–2.49 slices does not include:
