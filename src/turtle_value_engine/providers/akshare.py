...
_SOURCE_URIS = {
    ...
    "stock_szse_area_summary": "https://www.szse.cn/market/overview/index.html",
...
class AKShareProvider(StructuredDataProvider):
    ...
    def fetch_raw(self, request: ProviderRequest) -> RawProviderRecord:
        ...
        if (
            ...
            raise ProviderRequestError(
                "the AKShare H-share hot-rank-detail endpoint supports H-share "
                "listings only",
                provider=self.identity,
                request=request,
                retryable=False,
        ...
        if (
            ...
            raise ProviderRequestError(
                "the AKShare A-share hot-rank-detail endpoint supports A-share "
                "listings only",
                provider=self.identity,
                request=request,
                retryable=False,
        ...
        if (
            request.category is DataCategory.EARNINGS_QUICK_REPORT
            and listing.market is not ListingMarket.A
        ):
            raise ProviderRequestError(
                "the AKShare earnings-quick-report endpoint supports A-share listings only",
                provider=self.identity,
                request=request,
                retryable=False,
            )
        ...
        if (
            request.category is DataCategory.FINANCIAL_ABSTRACT
            and listing.market is not ListingMarket.A
        ):
            raise ProviderRequestError(
        ...
        if (
            ...
            rows = _table_rows(payload, provider=self.identity, request=request)
            _validate_market_quote_ah_comparison_provider_rows(
                rows,
                provider=self.identity,
                request=request,
            )
            selected = _select_market_quote_ah_comparison_rows(rows, listing)
            payload = selected
            response_metadata["upstream_row_count"] = len(rows)
            response_metadata["entity_row_count"] = len(selected)
            response_metadata["entity_rows_selected"] = True
            response_metadata["listing_scoped_request"] = False
            response_metadata["row_filtering"] = "provider"
            response_metadata["market_quote_view"] = _MARKET_QUOTE_AH_COMPARISON_VIEW
            response_metadata["snapshot_scope"] = "current_trading_day_delayed_15m"
            response_metadata["rank_field"] = "序号"
            response_metadata["rank_ordering"] = "strictly_ascending"
            response_metadata["date_binding"] = "retrieval_only"
            response_metadata["listing_code_field"] = (
                "A股代码" if listing.market is ListingMarket.A else "H股代码"
            )
            response_metadata["h_price_unit"] = "HKD_per_share"
            response_metadata["a_price_unit"] = "RMB_per_share"
            response_metadata["change_unit"] = "percent"
            response_metadata["comparison_unit"] = "ratio"
            ...
            if endpoint.name == "stock_board_industry_name_em":
                ...
                selected = _select_listing_rows(
                    ...
                    provider=self.identity,
                    request=request,
                    row_label="market-activity-statistic",
                )
                payload = selected
                response_metadata["upstream_row_count"] = len(rows)
                response_metadata["entity_row_count"] = len(selected)
                ...
                response_metadata["listing_scoped_request"] = False
                response_metadata["row_filtering"] = "provider"
                response_metadata["market_activity_view"] = _MARKET_ACTIVITY_STATISTIC_VIEW
                response_metadata["statistic_period"] = statistic_period
                response_metadata["upstream_symbol"] = kwargs["symbol"]
                response_metadata["date_binding"] = "request_period"
                response_metadata["snapshot_scope"] = "requested_statistic_period"
                response_metadata["observation_date_field"] = "最近上榜日"
                if recent_listing_dates:
                    response_metadata["observed_earliest_recent_listing_date"] = min(
            ...
            _validate_margin_trading_provider_rows(
                ...
                require_row_date=endpoint.name == "stock_margin_detail_sse",
                provider=self.identity,
                request=request,
            )
            ...
            response_metadata["date_binding"] = (
                "row_and_request" if endpoint.name == "stock_margin_detail_sse" else "request"
            )
            ...
            if endpoint.name == "stock_sy_em":
                ...
                _validate_goodwill_impairment_forecast_provider_rows(
                    rows,
                    provider=self.identity,
                    request=request,
                )
                selected = _select_listing_rows(
                    ...
                    provider=self.identity,
                    request=request,
                    row_label="goodwill-impairment forecast",
                )
                payload = selected
                response_metadata["upstream_row_count"] = len(rows)
                response_metadata["entity_row_count"] = len(selected)
                ...
                response_metadata["snapshot_scope"] = "requested_report_date"
                response_metadata["date_binding"] = "request_period"
                response_metadata["forecast_sequence_field"] = "序号"
                response_metadata["forecast_sequence_ordering"] = "strictly_ascending"
                response_metadata["forecast_date_fields"] = list(
                    _GOODWILL_IMPAIRMENT_FORECAST_DATE_FIELDS
                )
                response_metadata["amount_unit"] = "CNY"
                response_metadata["ratio_unit"] = "provider_reported_percent"
                response_metadata["amount_field_count"] = len(
                    _GOODWILL_IMPAIRMENT_FORECAST_AMOUNT_FIELDS
                )
                response_metadata["ratio_field_count"] = len(
                    _GOODWILL_IMPAIRMENT_FORECAST_RATIO_FIELDS
                )
                response_metadata["forecast_numeric_field_count"] = len(
                    _GOODWILL_IMPAIRMENT_FORECAST_NUMERIC_FIELDS
                )
                response_metadata["forecast_field_count"] = len(
                    _GOODWILL_IMPAIRMENT_FORECAST_FIELDS
                )
            elif endpoint.name == "stock_sy_profile_em":
                report_periods = _validate_goodwill_impairment_market_profile_provider_rows(
                    rows,
                    provider=self.identity,
                    request=request,
                )
                ...
                response_metadata["date_binding"] = "row_periods"
                response_metadata["amount_unit"] = "CNY"
                response_metadata["ratio_unit"] = "provider_reported_ratio"
                response_metadata["amount_field_count"] = len(
                    _GOODWILL_IMPAIRMENT_MARKET_PROFILE_AMOUNT_FIELDS
                )
                response_metadata["ratio_field_count"] = len(
                    _GOODWILL_IMPAIRMENT_MARKET_PROFILE_RATIO_FIELDS
                )
                response_metadata["market_profile_field_count"] = len(
                    _GOODWILL_IMPAIRMENT_MARKET_PROFILE_FIELDS
                )
                response_metadata["observation_start_date"] = (
                    min(report_periods).isoformat() if report_periods else None
                )
                response_metadata["observation_end_date"] = (
                ...
                raise ProviderResponseError(
                    "AKShare goodwill-impairment record came from an unsupported endpoint",
                    provider=self.identity,
                    request=request,
                )
            ...
            if endpoint.name == "stock_cyq_em":
                ...
                response_metadata["entity_rows_selected"] = True
                response_metadata["listing_scoped_request"] = True
                response_metadata["tencent_tick_view"] = _MARKET_HISTORY_TENCENT_TICK_VIEW
                response_metadata["upstream_symbol"] = kwargs["symbol"]
                response_metadata["snapshot_scope"] = "latest_trading_day"
                ...
                response_metadata["price_unit"] = "CNY_per_share"
                response_metadata["amount_unit"] = "CNY"
                response_metadata["amount_field"] = amount_field
                response_metadata["observation_start_time"] = (
                    min(observation_times).isoformat() if observation_times else None
                )
                response_metadata["observation_end_time"] = (
            ...
            elif endpoint.name == "stock_zh_a_hist_tx":
                ...
                observation_dates = _validate_tencent_daily_history_provider_rows(
                    rows,
                    start_date=start_date,
                    end_date=end_date,
                    provider=self.identity,
                ...
                response_metadata["entity_rows_selected"] = True
                response_metadata["listing_scoped_request"] = True
                response_metadata["tencent_daily_history_view"] = (
                    _MARKET_HISTORY_TENCENT_DAILY_VIEW
                )
                response_metadata["upstream_symbol"] = kwargs["symbol"]
                response_metadata["tencent_daily_start_date"] = kwargs["start_date"]
                response_metadata["tencent_daily_end_date"] = kwargs["end_date"]
                response_metadata["tencent_daily_adjust"] = kwargs["adjust"]
                response_metadata["date_binding"] = "row_and_request"
                response_metadata["range_filtering"] = "upstream_and_provider_validation"
                response_metadata["snapshot_scope"] = "requested_daily_range"
                response_metadata["observation_date_field"] = "date"
                response_metadata["volume_unit"] = "shares"
                response_metadata["amount_unit"] = "CNY"
                response_metadata["turnover_unit"] = "ratio"
                response_metadata["observation_start_date"] = (
                    min(observation_dates).isoformat() if observation_dates else None
                )
                response_metadata["observation_end_date"] = (
                ...
                observation_times = _validate_intraday_history_provider_rows(
                    rows,
                    period=kwargs["period"],
                    start_datetime=start_datetime,
                    end_datetime=end_datetime,
                ...
                response_metadata["entity_rows_selected"] = True
                response_metadata["listing_scoped_request"] = True
                response_metadata["intraday_history_view"] = _MARKET_HISTORY_INTRADAY_VIEW
                response_metadata["upstream_symbol"] = kwargs["symbol"]
                response_metadata["intraday_period"] = kwargs["period"]
                response_metadata["intraday_adjust"] = kwargs["adjust"]
                response_metadata["requested_start_datetime"] = kwargs["start_date"]
                response_metadata["requested_end_datetime"] = kwargs["end_date"]
                response_metadata["date_binding"] = "row_and_request"
                response_metadata["range_filtering"] = "upstream_and_provider_validation"
            ...
            selected = _select_listing_rows(
                ...
                provider=self.identity,
                request=request,
                row_label="earnings-quick-report",
            )
            payload = selected
            response_metadata["upstream_row_count"] = len(rows)
            response_metadata["entity_row_count"] = len(selected)
    ...
    def _resolve_endpoint(
        ...
        candidates = _endpoint_candidates(
            ...
            goodwill_impairment_forecast_requested=(
                request.parameters.get("view")
                == _GOODWILL_IMPAIRMENT_FORECAST_VIEW
            ),
            goodwill_impairment_market_profile_requested=(
                request.parameters.get("view")
                == _GOODWILL_IMPAIRMENT_MARKET_PROFILE_VIEW
            ),
            ...
            market_activity_szse_sector_summary_requested=(
                request.parameters.get("view")
                == _MARKET_ACTIVITY_SZSE_SECTOR_SUMMARY_VIEW
            ),
            market_activity_szse_summary_requested=(
                request.parameters.get("view")
                == _MARKET_ACTIVITY_SZSE_SUMMARY_VIEW
            ),
            market_activity_sse_deal_daily_requested=(
                request.parameters.get("view")
                == _MARKET_ACTIVITY_SSE_DEAL_DAILY_VIEW
            ),
            ...
            market_activity_limit_down_pool_requested=(
                request.parameters.get("view") == _MARKET_ACTIVITY_LIMIT_DOWN_POOL_VIEW
            ),
            ...
            market_activity_hk_hot_rank_detail_requested=(
                request.parameters.get("view")
                == _MARKET_ACTIVITY_HK_HOT_RANK_DETAIL_VIEW
            ),
    ...
    def _endpoint_kwargs(
        ...
        try:
            ...
            if request.category is DataCategory.EARNINGS_QUICK_REPORT:
            ...
            if request.category is DataCategory.FINANCIAL_ABSTRACT:
    ...
    def normalize(
        ...
        *,
        ...
        for record in ordered_records:
            ...
            if (
                ...
                if listing.market is not ListingMarket.A and not (
                    listing.market is ListingMarket.H
                    and (is_latest_hot_rank or is_hk_hot_rank_detail)
                ):
                    raise ProviderNormalizationError(
            ...
            elif record.request.category is DataCategory.EARNINGS_QUICK_REPORT:
                if listing.market is not ListingMarket.A:
                    raise ProviderNormalizationError(
                        "AKShare earnings-quick-report raw slice supports A-share listings only"
                    )
                try:
                    report_period = _parse_earnings_quick_report_date_parameter(
                        record.request.parameters.get("date"),
                        request=record.request,
                    )
                except ProviderRequestError as exc:
                    raise ProviderNormalizationError(str(exc)) from exc
                _validate_earnings_quick_report_normalizer_rows(
                    rows,
                    listing,
                    report_period=report_period,
                )
                # The quick-report headline does not identify parent versus
                # consolidated profit, and its per-share figures do not
                # establish the accepted entity, unit or diluted-share basis.
                missing_fields.update({"parent_net_profit", "consolidated_net_profit"})
                normalizer_flags.add("AKSHARE_EARNINGS_QUICK_REPORT_RAW_ONLY")
            ...
            elif record.request.category is DataCategory.FINANCIAL_ABSTRACT:
                if listing.market is not ListingMarket.A:
                    raise ProviderNormalizationError(
                        "AKShare financial-abstract raw slice supports A-share listings only"
                    )
                _validate_financial_abstract_normalizer_rows(rows, listing)
                # The abstract is a wide indicator matrix rather than a
                # statement-row contract. Its amount, per-share and ratio
                # rows do not establish the canonical entity, unit, period or
                # diluted-share basis for automatic fact mapping.
                missing_fields.update(
                    {
                ...
                try:
                    ...
                    raise ProviderNormalizationError(str(exc)) from exc
                _validate_financial_indicators_normalizer_rows(rows, listing)
                # The endpoint intentionally mixes reported amounts with
                # per-share values and provider-calculated ratios. Its report
                # presentation does not establish a canonical entity, unit or
                # calculation basis for automatic fact mapping.
                missing_fields.update(
                    {
                        "consolidated_net_profit",
                        "parent_net_profit",
                ...
                if endpoint_name == "stock_zh_a_gdhs_detail_em":
                    ...
                    try:
                        ...
                        raise ProviderNormalizationError(str(exc)) from exc
                    _validate_shareholder_count_normalizer_rows(
                        rows,
                        listing,
                        observation_date=observation_date,
                    )
                ...
                # canonical concentration metric or a diluted-share series.
                missing_fields.add("governance_risk_level")
            else:
                raise ProviderNormalizationError(
        ...
        notes = (
            ...
            "ranges and announcement dates are not reported profit. Earnings "
            "quick-report records remain raw structured evidence because their "
            "headline net profit and per-share figures do not establish the accepted "
            ...
            "Financial-abstract records remain raw structured evidence because "
            "their wide amount, per-share and ratio matrix does not establish "
            "the canonical entity, unit, period or diluted-share basis. "
            "Financial-indicator records remain raw structured evidence because "
            "their reported amounts, per-share values and provider ratios do not "
            "establish the canonical entity, unit or calculation basis. "
            "Dividend "
            ...
            "Shareholder-count records remain raw structured evidence because their "
            "quarter-end counts, average holdings and change percentages do not "
            "establish a canonical concentration metric, governance judgment or "
            ...
            "period, liquidity or valuation facts. Top-ten-shareholder records remain "
            "raw structured evidence because holder rank, quantities, ratios and "
            "report-date context do not establish beneficial control, canonical "
            "concentration or a company-level diluted-share series. "
            "A+H comparison snapshots also remain raw structured evidence because "
        ...
        if "AKSHARE_EARNINGS_QUICK_REPORT_RAW_ONLY" in normalizer_flags:
            notes += (
                " The documented A-share earnings-quick-report response is retained as "
                "raw evidence only: its headline net profit and per-share indicators "
                "do not establish the accepted parent/consolidated entity, unit or "
                "diluted-share basis."
            )
        ...
        if "AKSHARE_FINANCIAL_ABSTRACT_RAW_ONLY" in normalizer_flags:
            notes += (
                " The documented A-share financial-abstract response is retained "
                "as raw evidence only: its wide amount, per-share and ratio rows "
                "do not establish the canonical entity, unit, period or "
                "diluted-share basis."
            )
        ...
        if "AKSHARE_SSE_DEAL_DAILY_RAW_ONLY" in normalizer_flags:
            notes += (
                ...
                "its requested-date market aggregates and exchange-board breakdown do not "
                "establish a listing-level quote, issuer cash flow, shareholder return, "
        ...
        if "AKSHARE_SZSE_SUMMARY_RAW_ONLY" in normalizer_flags:
            notes += (
                ...
                "only: its requested-date security-category aggregates do not establish "
                "a listing-level quote, issuer cash flow, shareholder return, governance, "
                "valuation or canonical market metric; quantity and transaction-amount "
                "units are documented, while market-value units are not."
            )
        if "AKSHARE_SZSE_AREA_SUMMARY_RAW_ONLY" in normalizer_flags:
            notes += (
                " The documented SZSE area-summary response is retained as raw evidence "
                "only: its monthly region-level transaction aggregates do not establish "
                "a listing-level quote, issuer cash flow, shareholder return, governance, "
        ...
        if "AKSHARE_SZSE_SECTOR_SUMMARY_RAW_ONLY" in normalizer_flags:
            notes += (
                ...
                "only: its monthly or year-to-date industry transaction aggregates do not "
                "establish a listing-level quote, issuer cash flow, shareholder return, "
                "governance, valuation or canonical market metric; amount, share-count, "
                "transaction-count and percentage units remain explicit."
            )
        if "AKSHARE_MARKET_ACTIVITY_STATISTICS_RAW_ONLY" in normalizer_flags:
            notes += (
                ...
                "as raw evidence only: its per-listing activity counts, amount aggregates "
        ...
        if "AKSHARE_LIMIT_DOWN_POOL_RAW_ONLY" in normalizer_flags:
            notes += (
                " The documented A-share Eastmoney limit-down-pool response is retained "
                "as raw evidence only: its requested-date quote, limit-down activity and "
                "provider ranking fields do not establish issuer cash flow, shareholder "
                "return, governance, valuation or a canonical market metric."
            )
        ...
        if "AKSHARE_HK_HOT_RANK_LATEST_RAW_ONLY" in normalizer_flags:
            notes += (
                " The documented H-share stock latest-hot-rank response is retained as "
                "raw evidence only: its symbol-scoped popularity rank and provider "
                "timing fields do not establish issuer cash flow, shareholder return, "
                "governance, valuation or a canonical market metric."
            )
        if "AKSHARE_HK_HOT_RANK_DETAIL_RAW_ONLY" in normalizer_flags:
            notes += (
                " The documented H-share stock historical-hot-rank response is retained "
                "as raw evidence only: its date-bound popularity rank and provider "
                "security identity do not establish issuer cash flow, shareholder "
                "return, governance, valuation or a canonical market metric."
            )
        if "AKSHARE_HOT_RANK_DETAIL_RAW_ONLY" in normalizer_flags:
            notes += (
                ...
                "as raw evidence only: its date-bound popularity rank and follower "
                "ratios do not establish issuer cash flow, shareholder return, "
                "governance, valuation or a canonical market metric."
            )
        if "AKSHARE_TOP_10_SHAREHOLDERS_RAW_ONLY" in normalizer_flags:
            notes += (
                ...
                "raw evidence only: report-period rank, holder, share type, quantity "
        ...
        if "AKSHARE_SHAREHOLDER_COUNTS_RAW_ONLY" in normalizer_flags:
            notes += (
                ...
                "raw evidence only: its quarter-end shareholder counts, average "
                "holdings and change percentages do not establish a canonical "
        ...
        if "AKSHARE_GOODWILL_FORECAST_RAW_ONLY" in normalizer_flags:
            notes += (
                ...
                "retained as raw evidence only: expected profit ranges, goodwill "
                "context and announcement dates do not establish filing-backed "
                "accounting scope or canonical goodwill/impairment facts."
            )
        if "AKSHARE_GOODWILL_PROFILE_RAW_ONLY" in normalizer_flags:
            notes += (
                " The documented A-share goodwill market-profile history is retained "
                "as raw evidence only: its market-wide aggregate amounts, provider "
        ...
        if "AKSHARE_AH_COMPARISON_RAW_ONLY" in normalizer_flags:
            notes += (
                " The documented A+H comparison response is retained as raw evidence "
                "only: its delayed cross-market prices, changes, ratio and premium "
        ...
        if "AKSHARE_TENCENT_TICK_RAW_ONLY" in normalizer_flags:
            notes += (
                ...
                "evidence only: its latest-trading-day time-only trade rows do not "
        ...
        if "AKSHARE_INTRADAY_HISTORY_RAW_ONLY" in normalizer_flags:
            notes += (
                ...
                "evidence only: minute-bar interval, adjustment mode and recent-data "
...
_MARKET_ACTIVITY_STATISTIC_DATE_FIELDS = (
    "最近上榜日",
    "LATEST_TDATE",
    "latest_tdate",
    "date",
    ...
    *,
    ...
    if category is DataCategory.MARKET_HISTORY:
        if market is ListingMarket.A:
            ...
            if market_history_tencent_tick_requested:
    ...
    if category is DataCategory.MARKET_ACTIVITY:
        ...
        if market_activity_szse_sector_summary_requested:
            ...
            return ()
        if market_activity_szse_area_summary_requested:
            if market is ListingMarket.A:
                return ("stock_szse_area_summary",)
            return ()
        if market_activity_szse_summary_requested:
            if market is ListingMarket.A:
        ...
        if market_activity_sse_deal_daily_requested:
            ...
            return ()
        if market_activity_hot_rank_detail_requested:
            if market is ListingMarket.A:
                return (_MARKET_ACTIVITY_HOT_RANK_DETAIL_ENDPOINT,)
            return ()
        if market_activity_hk_hot_rank_detail_requested:
            if market is ListingMarket.H:
                return (_MARKET_ACTIVITY_HK_HOT_RANK_DETAIL_ENDPOINT,)
            return ()
        ...
        if market is ListingMarket.A:
            ...
            if market_activity_limit_down_pool_requested:
                return ("stock_zt_pool_dtgc_em",)
    ...
    if category is DataCategory.EARNINGS_FORECAST:
        ...
        return ()
    if category is DataCategory.EARNINGS_QUICK_REPORT:
        if market is ListingMarket.A:
    ...
    if category is DataCategory.BUSINESS_COMPOSITION:
        ...
        return ()
    if category is DataCategory.FINANCIAL_ABSTRACT:
        if market is ListingMarket.A:
...
def _market_quote_kwargs(
    ...
    if endpoint_name == "stock_zh_ah_spot_em":
        ...
        if unknown:
            raise ProviderRequestError(
                "unsupported AKShare A+H comparison parameter(s): "
                + ", ".join(unknown),
                request=request,
                retryable=False,
            )
        if request.parameters.get("view") != _MARKET_QUOTE_AH_COMPARISON_VIEW:
            raise ProviderRequestError(
...
def _financial_abstract_kwargs(
    endpoint_name: str,
    listing: _ListingRef,
    request: ProviderRequest,
) -> dict[str, object]:
    if endpoint_name != "stock_financial_abstract":
        raise ProviderRequestError(
            f"unsupported AKShare financial-abstract endpoint {endpoint_name!r}",
            request=request,
            retryable=False,
        )
    if listing.market is not ListingMarket.A:
    ...
    unknown = sorted(set(request.parameters) - _FINANCIAL_ABSTRACT_PARAMETER_NAMES)
    if unknown:
        raise ProviderRequestError(
            "unsupported AKShare financial-abstract parameter(s): " + ", ".join(unknown),
            request=request,
            retryable=False,
        )
    return {"symbol": listing.code}
...
def _financial_indicators_kwargs(
    endpoint_name: str,
    listing: _ListingRef,
    request: ProviderRequest,
) -> dict[str, object]:
    ...
    unknown = sorted(set(request.parameters) - _FINANCIAL_INDICATORS_PARAMETER_NAMES)
    if unknown:
        raise ProviderRequestError(
            "unsupported AKShare financial-indicators parameter(s): " + ", ".join(unknown),
            request=request,
            retryable=False,
        )
...
def _earnings_quick_report_kwargs(
    ...
    if endpoint_name != "stock_yjkb_em":
        raise ProviderRequestError(
            f"unsupported AKShare earnings-quick-report endpoint {endpoint_name!r}",
            request=request,
            retryable=False,
        )
    if listing.market is not ListingMarket.A:
    ...
    unknown = sorted(set(request.parameters) - _EARNINGS_QUICK_REPORT_PARAMETER_NAMES)
    if unknown:
        raise ProviderRequestError(
            "unsupported AKShare earnings-quick-report parameter(s): " + ", ".join(unknown),
            request=request,
            retryable=False,
        )
    if "date" not in request.parameters:
        raise ProviderRequestError(
            "the AKShare earnings-quick-report endpoint requires date (YYYYMMDD)",
            request=request,
            retryable=False,
        )
    report_date = _parse_earnings_quick_report_date_parameter(
        request.parameters["date"],
        request=request,
    )
    return {"date": report_date.strftime("%Y%m%d")}
...
def _shareholder_holdings_kwargs(
    ...
    if endpoint_name == "stock_hold_num_cninfo":
        if listing.market is not ListingMarket.A:
            raise ProviderRequestError(
                "the AKShare shareholder-count endpoint supports A-share listings only",
                request=request,
                retryable=False,
            )
        unknown = sorted(set(request.parameters) - _SHAREHOLDER_COUNT_PARAMETER_NAMES)
        if unknown:
            raise ProviderRequestError(
                "unsupported AKShare shareholder-count parameter(s): "
                + ", ".join(unknown),
                request=request,
                retryable=False,
            )
        if "date" not in request.parameters:
            raise ProviderRequestError(
                "the AKShare shareholder-count endpoint requires date (YYYYMMDD)",
                request=request,
                retryable=False,
            )
        raw_date = request.parameters["date"]
        _parse_shareholder_count_date_parameter(raw_date, request=request)
        return {"date": raw_date}
...
def _goodwill_impairment_kwargs(
    ...
    if endpoint_name == "stock_sy_yq_em":
        if listing.market is not ListingMarket.A:
            raise ProviderRequestError(
                "the AKShare goodwill-impairment forecast endpoint supports "
                "A-share listings only",
                request=request,
                retryable=False,
            )
        unknown = sorted(
            set(request.parameters) - _GOODWILL_IMPAIRMENT_FORECAST_PARAMETER_NAMES
        )
        if unknown:
            raise ProviderRequestError(
                "unsupported AKShare goodwill-impairment forecast parameter(s): "
                + ", ".join(unknown),
                request=request,
                retryable=False,
            )
        if request.parameters.get("view") != _GOODWILL_IMPAIRMENT_FORECAST_VIEW:
            raise ProviderRequestError(
                "goodwill-impairment forecast view must be "
                f"{_GOODWILL_IMPAIRMENT_FORECAST_VIEW!r}",
                request=request,
                retryable=False,
            )
        if "date" not in request.parameters:
            raise ProviderRequestError(
                "the AKShare goodwill-impairment forecast endpoint requires "
                "date (YYYYMMDD)",
                request=request,
                retryable=False,
            )
        ...
        _parse_goodwill_impairment_date_parameter(raw_date, request=request)
        return {"date": raw_date}
    if endpoint_name == "stock_sy_profile_em":
        if listing.market is not ListingMarket.A:
            raise ProviderRequestError(
                "the AKShare goodwill-impairment market-profile endpoint supports "
                "A-share listings only",
                request=request,
                retryable=False,
            )
        unknown = sorted(
            set(request.parameters)
            - _GOODWILL_IMPAIRMENT_MARKET_PROFILE_PARAMETER_NAMES
        )
        if unknown:
            raise ProviderRequestError(
                "unsupported AKShare goodwill-impairment market-profile parameter(s): "
                + ", ".join(unknown),
                request=request,
                retryable=False,
            )
        if request.parameters.get("view") != _GOODWILL_IMPAIRMENT_MARKET_PROFILE_VIEW:
            raise ProviderRequestError(
                "goodwill-impairment market-profile view must be "
                f"{_GOODWILL_IMPAIRMENT_MARKET_PROFILE_VIEW!r}",
                request=request,
                retryable=False,
            )
        return {}
...
def _parse_shareholder_count_date_parameter(
    raw_value: object,
    *,
    request: ProviderRequest | None = None,
) -> date:
    if not isinstance(raw_value, str) or not re.fullmatch(r"\d{8}", raw_value):
        raise ProviderRequestError(
            "shareholder-count date must be YYYYMMDD",
            request=request,
            retryable=False,
        )
    try:
        ...
        raise ProviderRequestError(
            "shareholder-count date must be a valid YYYYMMDD date",
            request=request,
            retryable=False,
        ) from exc
    if parsed < _SHAREHOLDER_COUNT_START_DATE:
        raise ProviderRequestError(
            "shareholder-count date must be on or after 20170331",
            request=request,
            retryable=False,
        )
    if (parsed.month, parsed.day) not in _SHAREHOLDER_COUNT_QUARTER_ENDS:
        raise ProviderRequestError(
            "shareholder-count date must be an exact quarter-end report date",
            request=request,
            retryable=False,
        )
    return parsed
...
def _parse_earnings_quick_report_date_parameter(
    raw_value: object,
    *,
    request: ProviderRequest | None = None,
) -> date:
    if not isinstance(raw_value, str) or not re.fullmatch(r"\d{8}", raw_value):
        raise ProviderRequestError(
            "earnings-quick-report date must be YYYYMMDD",
            request=request,
            retryable=False,
        )
    try:
        ...
        raise ProviderRequestError(
            "earnings-quick-report date must be a valid YYYYMMDD date",
            request=request,
            retryable=False,
        ) from exc
    if parsed < _EARNINGS_QUICK_REPORT_START_DATE:
        raise ProviderRequestError(
            "earnings-quick-report date must be on or after 20100331",
            request=request,
            retryable=False,
        )
    if (parsed.month, parsed.day) not in _EARNINGS_QUICK_REPORT_QUARTER_ENDS:
        raise ProviderRequestError(
            "earnings-quick-report date must be an exact quarter-end report date",
            request=request,
            retryable=False,
        )
    return parsed
...
def _market_quote_ah_comparison_validation_message(
    ...
    for index, row in enumerate(rows):
        ...
        for field in _MARKET_QUOTE_AH_COMPARISON_NUMERIC_FIELDS:
            ...
            if isinstance(value, bool) or not isinstance(value, Real):
                return (
                    f"A+H comparison row {index} field {field!r} must be numeric or null"
                )
            try:
                numeric = float(value)
            except (OverflowError, TypeError, ValueError):
            ...
            if not math.isfinite(numeric):
                return (
                    f"A+H comparison row {index} field {field!r} must be finite or null"
                )
    return None
    ...
    *,
    ...
    for index, row in enumerate(rows):
        ...
        for field in numeric_fields:
            ...
            if isinstance(value, bool) or not isinstance(value, Real):
                return (
                    f"intraday-history row {index} field {field!r} must be numeric or null",
                    [],
                )
            try:
                numeric = float(value)
            ...
            if not math.isfinite(numeric):
                return (
                    f"intraday-history row {index} field {field!r} must be finite or null",
                    [],
                )
    ...
    *,
    ...
    for index, row in enumerate(rows):
        ...
        for field in numeric_fields:
            ...
            if isinstance(value, bool) or not isinstance(value, Real):
                return (
                    f"Tencent daily-history row {index} field {field!r} must be numeric or null",
                    [],
                )
            try:
                numeric = float(value)
            ...
            if not math.isfinite(numeric):
                return (
                    f"Tencent daily-history row {index} field {field!r} must be finite or null",
                    [],
                )
...
def _tencent_tick_validation_message(
    ...
    for index, row in enumerate(rows):
        ...
        for field in ("成交价格", "价格变动", "成交量", amount_field):
            ...
            if isinstance(value, bool) or not isinstance(value, Real):
                return (
                    f"Tencent tick row {index} field {field!r} must be numeric or null",
                    [],
                    amount_field,
                )
            try:
            ...
            if not math.isfinite(numeric):
                return (
                    f"Tencent tick row {index} field {field!r} must be finite or null",
                    [],
                    amount_field,
                )
...
def _validate_intraday_history_normalizer_scope(
    ...
    expected_metadata = {
        ...
        "upstream_symbol": upstream_kwargs["symbol"],
        "intraday_period": upstream_kwargs["period"],
        "intraday_adjust": upstream_kwargs["adjust"],
    ...
    for name, expected in expected_metadata.items():
        if record.response_metadata.get(name) != expected:
            raise ProviderNormalizationError(
                f"intraday-history response metadata {name!r} does not match "
                "the requested replay scope"
            )
    ...
    if record.response_metadata.get("observation_start_datetime") != expected_start:
        raise ProviderNormalizationError(
            "intraday-history response observation start does not match replayed rows"
        )
...
def _market_activity_szse_summary_validation_message(
    ...
    for index, row in enumerate(rows):
        ...
        for field in _MARKET_ACTIVITY_SZSE_SUMMARY_NUMERIC_FIELDS:
            ...
            if isinstance(value, bool) or not isinstance(value, Real):
                return (
                    f"SZSE market-summary row {index} field {field!r} must be numeric or null",
                    [],
                )
            try:
                numeric = float(value)
            ...
            if not math.isfinite(numeric):
                return (
                    f"SZSE market-summary row {index} field {field!r} must be finite or null",
                    [],
                )
...
def _market_activity_sse_deal_daily_validation_message(
    ...
    for index, row in enumerate(rows):
        ...
        for field in _MARKET_ACTIVITY_SSE_DEAL_DAILY_NUMERIC_FIELDS:
            ...
            if isinstance(value, bool) or not isinstance(value, Real):
                return (
                    f"SSE daily-deal row {index} field {field!r} must be numeric or null"
                )
            try:
                numeric = float(value)
            except (OverflowError, TypeError, ValueError):
            ...
            if not math.isfinite(numeric):
                return (
                    f"SSE daily-deal row {index} field {field!r} must be finite or null"
                )
    return None
...
def _market_activity_limit_down_pool_validation_message(
    ...
    for index, row in enumerate(rows):
        ...
        if row_code in seen_codes:
            return (
                "market-activity limit-down-pool response has duplicate listing code "
                f"{row_code!r}"
            )
        seen_codes.add(row_code)
...
def _market_activity_hot_rank_detail_validation_message(
    ...
    for index, row in enumerate(rows):
        ...
        if previous_date is not None and observation_date <= previous_date:
            if observation_date == previous_date:
                return (
                    "market-activity A-share hot-rank-detail response has duplicate 时间 "
                    f"{observation_date.isoformat()!r}",
                    [],
                )
            return (
                "market-activity A-share hot-rank-detail response 时间 values must be "
                "strictly ascending",
                [],
            )
        previous_date = observation_date
...
def _market_activity_hk_hot_rank_detail_validation_message(
    ...
    for index, row in enumerate(rows):
        ...
        if previous_date is not None and observation_date <= previous_date:
            if observation_date == previous_date:
                return (
                    "market-activity H-share hot-rank-detail response has duplicate 时间 "
                    f"{observation_date.isoformat()!r}",
                    [],
                )
            return (
                "market-activity H-share hot-rank-detail response 时间 values must be "
                "strictly ascending",
                [],
            )
        previous_date = observation_date
...
def _market_activity_hot_rank_latest_validation_message(
    ...
    listing: _ListingRef | None = None,
    *,
    endpoint_name: str = "stock_hot_rank_latest_em",
) -> tuple[str | None, datetime | None]:
    ...
    if endpoint_name not in _MARKET_ACTIVITY_HOT_RANK_LATEST_ENDPOINTS:
        return (
            "market-activity latest-hot-rank response uses an unsupported endpoint",
            None,
        )
    expected_market = (
        ListingMarket.H
    ...
    expected_market_type = _MARKET_ACTIVITY_HOT_RANK_LATEST_MARKET_TYPES[endpoint_name]
    if listing is not None and listing.market is not expected_market:
        return (
            "market-activity latest-hot-rank endpoint supports "
            f"{expected_market.value}-share listings only",
            None,
        )
    ...
    for index, row in enumerate(rows):
        ...
        if item in _MARKET_ACTIVITY_HOT_RANK_LATEST_TEXT_ITEMS:
            ...
            if item == "marketType":
                if value != expected_market_type:
                    return (
                        "market-activity latest-hot-rank item 'marketType' must be "
                        f"{expected_market_type!r} for the "
                        f"{expected_market.value}-share endpoint",
                        None,
                    )
            elif item == "calcTime":
                observation_time = _intraday_history_timestamp(value)
                ...
                if endpoint_name == "stock_hk_hot_rank_latest_em":
                    if not re.fullmatch(r"\d{5}(?:_\d+)?", value):
                        return (
                            "market-activity latest-hot-rank item 'innerCode' must be "
                            "a five-digit H-share security code",
                            None,
                        )
                    if listing is not None and value[:5] != listing.code:
                        return (
                            "market-activity latest-hot-rank item 'innerCode' "
                            f"{value!r} does not match requested listing "
                            f"{listing.code!r}",
                            None,
                        )
                elif not re.fullmatch(r"\d{6}", value):
                    return (
                ...
                if endpoint_name == "stock_hk_hot_rank_latest_em":
                    valid_security_code = re.fullmatch(r"HK\|\d{5}", normalized_code)
                    expected_security_code = (
                        None if listing is None else f"HK|{listing.code}"
                    )
                    invalid_message = (
                        "must be an HK| five-digit H-share code"
                    )
                else:
                    valid_security_code = re.fullmatch(
                        r"(?:SH|SZ|BJ)\d{6}", normalized_code
                    )
                    expected_security_code = (
                        None if listing is None else listing.canonical_id
                    )
                    invalid_message = "must be a market-prefixed A-share code"
                if valid_security_code is None:
                    return (
                ...
                if (
                    listing is not None
                    and normalized_code != expected_security_code
                ):
                    return (
                        "market-activity latest-hot-rank item 'srcSecurityCode' "
                        f"{value!r} does not match requested listing "
...
def _validate_market_activity_hot_rank_latest_provider_rows(
    ...
    *,
    ...
    """Validate an A/H symbol-scoped latest popularity-rank response."""
...
def _goodwill_impairment_forecast_validation_message(
    ...
    """Return strict-schema errors for goodwill-impairment forecast rows."""

    previous_sequence: int | None = None
    for index, row in enumerate(rows):
        missing = sorted(_GOODWILL_IMPAIRMENT_FORECAST_FIELDS - set(row))
        unexpected = sorted(set(row) - _GOODWILL_IMPAIRMENT_FORECAST_FIELDS)
        if missing:
            return (
                "goodwill-impairment forecast row "
        ...
        if (
            ...
            return (
                f"goodwill-impairment forecast row {index} field '序号' "
                "must be a positive integer"
            )
        if previous_sequence is not None and sequence <= previous_sequence:
            if sequence == previous_sequence:
                return (
                    "goodwill-impairment forecast response has duplicate 序号 "
                    f"{sequence!r}"
                )
            return (
                "goodwill-impairment forecast response 序号 values must be "
                "strictly ascending"
            )
        previous_sequence = sequence
        ...
        if _row_code(row, ListingMarket.A) is None:
            return (
                f"goodwill-impairment forecast row {index} has no explicit listing code"
            )

        for field in _GOODWILL_IMPAIRMENT_FORECAST_DATE_FIELDS:
            value = row[field]
            if value is not None and _parse_date_value(value) is None:
                return (
                    f"goodwill-impairment forecast row {index} has an invalid {field}"
                )

        for field in _GOODWILL_IMPAIRMENT_FORECAST_NUMERIC_FIELDS:
            value = row[field]
            if value is None:
                continue
            if isinstance(value, bool) or not isinstance(value, Real):
        ...
        for field in _GOODWILL_IMPAIRMENT_FORECAST_TEXT_FIELDS:
            value = row[field]
            if value is not None and not isinstance(value, str):
                return (
                    "goodwill-impairment forecast row "
                    f"{index} field {field!r} must be a string or null"
                )
    return None


def _goodwill_impairment_market_profile_validation_message(
    rows: Sequence[Mapping[str, JSONValue]],
) -> tuple[str | None, list[date]]:
    """Return strict-schema errors for the market-wide goodwill history."""

    report_periods: list[date] = []
    previous_period: date | None = None
    for index, row in enumerate(rows):
        missing = sorted(_GOODWILL_IMPAIRMENT_MARKET_PROFILE_FIELDS - set(row))
        unexpected = sorted(set(row) - _GOODWILL_IMPAIRMENT_MARKET_PROFILE_FIELDS)
        if missing:
            return (
                "goodwill-impairment market-profile row "
                f"{index} is missing field(s): "
                + ", ".join(missing),
                [],
            )
        ...
        report_period = _parse_date_value(row["报告期"])
        if report_period is None:
            return (
                f"goodwill-impairment market-profile row {index} has an invalid 报告期",
                [],
            )
        if previous_period is not None and report_period <= previous_period:
            if report_period == previous_period:
                return (
                    "goodwill-impairment market-profile response has duplicate 报告期 "
                    f"{report_period.isoformat()!r}",
                    [],
                )
            return (
                "goodwill-impairment market-profile response 报告期 values must be "
                "strictly ascending",
                [],
            )
        previous_period = report_period
        report_periods.append(report_period)

        for field in _GOODWILL_IMPAIRMENT_MARKET_PROFILE_NUMERIC_FIELDS:
            value = row[field]
            if value is None:
                continue
            if isinstance(value, bool) or not isinstance(value, Real):
    ...
    return None, report_periods


def _validate_goodwill_impairment_market_profile_provider_rows(
    rows: Sequence[Mapping[str, JSONValue]],
    *,
    provider: ProviderIdentity,
    request: ProviderRequest,
) -> list[date]:
    """Validate the market-wide goodwill history before storage."""
...
def _validate_goodwill_impairment_forecast_provider_rows(
    ...
    *,
    ...
    """Validate the A-share goodwill-forecast universe before filtering."""
...
def _validate_financial_abstract_provider_rows(
    rows: Sequence[Mapping[str, JSONValue]],
    listing: _ListingRef,
    *,
    provider: ProviderIdentity,
    ...
    """Validate metric identity and report-period columns before storage."""

    for row in rows:
        row_code = _row_code(row, ListingMarket.A)
        if row_code is not None and row_code != listing.code:
            raise ProviderResponseError(
                f"AKShare returned financial-abstract row entity {row_code!r} for "
                f"requested listing {listing.canonical_id!r}",
                provider=provider,
                request=request,
            )
        ...
        if (
            ...
            raise ProviderResponseError(
                f"AKShare returned a financial-abstract row without explicit metric "
                f"identity for {request.entity_id!r}",
                provider=provider,
                request=request,
            )
        for key in row:
            if not _is_financial_abstract_period_key(key):
                continue
            if _parse_date_value(key) is None:
                raise ProviderResponseError(
                    f"AKShare returned an invalid financial-abstract report-period "
                    f"column {key!r} for {request.entity_id!r}",
                    provider=provider,
                    request=request,
                )


def _validate_financial_indicators_provider_rows(
    rows: Sequence[Mapping[str, JSONValue]],
    listing: _ListingRef,
    *,
    provider: ProviderIdentity,
    ...
    for row in rows:
        ...
        if row_code != listing.code:
            raise ProviderResponseError(
                f"AKShare returned financial-indicator row entity {row_code!r} for "
                f"requested listing {listing.canonical_id!r}",
                provider=provider,
                request=request,
            )
        ...
        if not found or _text_value(raw_period) in _MISSING_TEXT:
            raise ProviderResponseError(
                f"AKShare returned a financial-indicator row without a report date for "
                f"{request.entity_id!r}",
                provider=provider,
                request=request,
            )
        if _parse_date_value(raw_period) is None:
            raise ProviderResponseError(
                f"AKShare returned an invalid financial-indicator report date for "
                f"{request.entity_id!r}",
                provider=provider,
                request=request,
            )
...
def _validate_earnings_quick_report_provider_rows(
    ...
    *,
    ...
    """Validate an earnings-quick-report universe before filtering its rows."""

    for row in rows:
        if _row_code(row, ListingMarket.A) is None:
            raise ProviderResponseError(
                f"AKShare returned an earnings-quick-report row without a listing code for "
                f"{request.entity_id!r}",
                provider=provider,
                request=request,
            )
        ...
        if row_period is None:
            raise ProviderResponseError(
                f"AKShare returned an earnings-quick-report row with an invalid report date "
                f"for {request.entity_id!r}",
                provider=provider,
                request=request,
            )
        if row_period != report_period:
            raise ProviderResponseError(
                f"AKShare returned earnings-quick-report row period {row_period.isoformat()!r}; "
                f"requested {report_period.isoformat()!r}",
                provider=provider,
                request=request,
            )
...
def _validate_market_activity_szse_area_summary_normalizer_scope(
    ...
    """Validate replay scope for the monthly SZSE area-ranked report."""

    if listing.market is not ListingMarket.A:
        raise ProviderNormalizationError(
            "AKShare SZSE area-summary raw slice supports A-share listings only"
        )
    endpoint_name = "stock_szse_area_summary"
    if record.response_metadata.get("endpoint") != endpoint_name:
        raise ProviderNormalizationError(
            "AKShare SZSE area-summary record must come from stock_szse_area_summary"
        )
    if record.source_uri != _SOURCE_URIS[endpoint_name]:
        raise ProviderNormalizationError(
            "AKShare SZSE area-summary source URI does not match the documented endpoint"
        )
    try:
    ...
    for name, expected in expected_metadata.items():
        ...
        if not matches:
            raise ProviderNormalizationError(
                f"AKShare SZSE area-summary response metadata {name!r} does not "
                "match the requested replay scope"
            )
...
def _validate_market_activity_szse_sector_summary_normalizer_scope(
    ...
    """Validate replay scope for the monthly SZSE sector report."""

    if listing.market is not ListingMarket.A:
        raise ProviderNormalizationError(
            "AKShare SZSE sector-summary raw slice supports A-share listings only"
        )
    endpoint_name = "stock_szse_sector_summary"
    if record.response_metadata.get("endpoint") != endpoint_name:
        raise ProviderNormalizationError(
            "AKShare SZSE sector-summary record must come from "
            "stock_szse_sector_summary"
        )
    if record.source_uri != _SOURCE_URIS[endpoint_name]:
        raise ProviderNormalizationError(
            "AKShare SZSE sector-summary source URI does not match the documented endpoint"
        )
    try:
    ...
    for name, expected in expected_metadata.items():
        ...
        if not matches:
            raise ProviderNormalizationError(
                f"AKShare SZSE sector-summary response metadata {name!r} does not "
                "match the requested replay scope"
            )
...
def _validate_market_activity_szse_summary_normalizer_scope(
    ...
    """Validate replay scope for the market-level SZSE security report."""

    if listing.market is not ListingMarket.A:
        raise ProviderNormalizationError(
            "AKShare SZSE market-summary raw slice supports A-share listings only"
        )
    endpoint_name = "stock_szse_summary"
    if record.response_metadata.get("endpoint") != endpoint_name:
        raise ProviderNormalizationError(
            "AKShare SZSE market-summary record must come from stock_szse_summary"
        )
    if record.source_uri != _SOURCE_URIS[endpoint_name]:
        raise ProviderNormalizationError(
            "AKShare SZSE market-summary source URI does not match the documented endpoint"
        )
    try:
    ...
    for name, expected in expected_metadata.items():
        ...
        if not matches:
            raise ProviderNormalizationError(
                f"AKShare SZSE market-summary response metadata {name!r} does not "
                "match the requested replay scope"
            )
...
def _validate_market_activity_sse_deal_daily_normalizer_scope(
    ...
    """Validate replay scope for the market-wide SSE daily-deal overview."""

    if listing.market is not ListingMarket.A:
        raise ProviderNormalizationError(
            "AKShare SSE daily-deal raw slice supports A-share listings only"
        )
    endpoint_name = "stock_sse_deal_daily"
    if record.response_metadata.get("endpoint") != endpoint_name:
        raise ProviderNormalizationError(
            "AKShare SSE daily-deal record must come from stock_sse_deal_daily"
        )
    if record.source_uri != _SOURCE_URIS[endpoint_name]:
        raise ProviderNormalizationError(
            "AKShare SSE daily-deal source URI does not match the documented endpoint"
        )
    try:
...
def _validate_market_activity_limit_down_pool_normalizer_scope(
    ...
    """Validate the replay scope of a filtered limit-down-pool snapshot."""

    if listing.market is not ListingMarket.A:
        raise ProviderNormalizationError(
            "AKShare market-activity limit-down-pool raw slice supports A-share listings "
            "only"
        )
    if record.response_metadata.get("endpoint") != "stock_zt_pool_dtgc_em":
        raise ProviderNormalizationError(
            "AKShare market-activity limit-down-pool record must come from "
            "stock_zt_pool_dtgc_em"
        )
    if record.source_uri != _SOURCE_URIS["stock_zt_pool_dtgc_em"]:
        raise ProviderNormalizationError(
            "market-activity limit-down-pool source URI does not match the documented "
            "endpoint"
        )
    try:
    ...
    expected_metadata = {
        ...
        "listing_scoped_request": False,
        "row_filtering": "provider",
        "snapshot_scope": "requested_trading_date_limit_down_pool",
        "date_binding": "request",
        "rank_field": "序号",
        "rank_ordering": "strictly_ascending",
        "code_field": "代码",
    ...
    for name, expected in expected_metadata.items():
        ...
        if not matches:
            raise ProviderNormalizationError(
                f"market-activity limit-down-pool response metadata {name!r} does not "
                "match the requested replay scope"
            )
    ...
    if (
        ...
        raise ProviderNormalizationError(
            "market-activity limit-down-pool response upstream row count does not "
            "match the requested replay scope"
        )
...
def _validate_market_activity_hk_hot_rank_detail_normalizer_scope(
    ...
    for name, expected in expected_metadata.items():
        ...
        if not matches:
            raise ProviderNormalizationError(
                "market-activity H-share hot-rank-detail response metadata "
...
def _validate_market_activity_hot_rank_detail_normalizer_scope(
    ...
    for name, expected in expected_metadata.items():
        ...
        if not matches:
            raise ProviderNormalizationError(
                "market-activity A-share hot-rank-detail response metadata "
                f"{name!r} does not match the requested replay scope"
            )
...
def _validate_market_quote_ah_comparison_normalizer_scope(
    ...
    """Validate the replay scope of a filtered A+H comparison snapshot."""

    if record.response_metadata.get("endpoint") != "stock_zh_ah_spot_em":
        raise ProviderNormalizationError(
            "A+H comparison record must come from stock_zh_ah_spot_em"
        )
    if record.response_metadata.get("market") != listing.market.value:
        raise ProviderNormalizationError(
            "A+H comparison response market does not match requested listing"
        )
    if record.response_metadata.get("listing_code") != listing.code:
        raise ProviderNormalizationError(
            "A+H comparison response listing code does not match requested listing"
        )
    try:
        _market_quote_kwargs(
            "stock_zh_ah_spot_em",
    ...
    expected_metadata = {
        ...
        "h_price_unit": "HKD_per_share",
        "a_price_unit": "RMB_per_share",
        "change_unit": "percent",
        "comparison_unit": "ratio",
        "premium_unit": "percent",
        "entity_rows_selected": True,
        "entity_row_count": len(rows),
    }
    boolean_fields = {"listing_scoped_request", "entity_rows_selected"}
    ...
    for name, expected in expected_metadata.items():
        ...
        if not matches:
            raise ProviderNormalizationError(
                f"A+H comparison response metadata {name!r} does not match "
                "the requested replay scope"
            )
    ...
    if (
        ...
        raise ProviderNormalizationError(
            "A+H comparison response upstream row count does not match "
            "the requested replay scope"
        )
...
def _validate_market_activity_statistic_normalizer_rows(
    ...
    """Keep replayed Dragon-Tiger statistic rows inside their listing scope."""
    ...
    for row in rows:
        ...
        if row_code is None:
            raise ProviderNormalizationError(
                "market-activity-statistic row has no explicit listing code"
            )
        if row_code != listing.code:
            raise ProviderNormalizationError(
                f"market-activity-statistic row entity {row_code!r} does not match "
                f"requested listing {listing.canonical_id!r}"
            )
        if row_code in seen_codes:
            raise ProviderNormalizationError(
                f"market-activity-statistic row has duplicate listing code {row_code!r}"
            )
        seen_codes.add(row_code)
...
def _validate_goodwill_impairment_forecast_normalizer_scope(
    ...
    """Validate replayed scope and metadata for the dated forecast universe."""

    if listing.market is not ListingMarket.A:
        raise ProviderNormalizationError(
            "AKShare goodwill-impairment forecast raw slice supports A-share listings "
            "only"
        )
    if record.response_metadata.get("endpoint") != "stock_sy_yq_em":
        raise ProviderNormalizationError(
            "AKShare goodwill-impairment forecast record must come from stock_sy_yq_em"
        )
    if record.source_uri != _SOURCE_URIS["stock_sy_yq_em"]:
        raise ProviderNormalizationError(
            "AKShare goodwill-impairment forecast source URI does not match the "
            "documented endpoint"
        )
    try:
        upstream_kwargs = _goodwill_impairment_kwargs(
    ...
    boolean_fields = {"listing_scoped_request", "entity_rows_selected"}
    count_fields = {
        "forecast_numeric_field_count",
        "forecast_field_count",
        "amount_field_count",
        "ratio_field_count",
        "entity_row_count",
    }
...
def _validate_goodwill_impairment_market_profile_normalizer_scope(
    ...
    """Validate replayed scope and rows for the market-wide goodwill history."""

    if listing.market is not ListingMarket.A:
        raise ProviderNormalizationError(
            "AKShare goodwill-impairment market-profile raw slice supports A-share "
            "listings only"
        )
    if record.response_metadata.get("endpoint") != "stock_sy_profile_em":
        raise ProviderNormalizationError(
            "AKShare goodwill-impairment market-profile record must come from "
            "stock_sy_profile_em"
        )
    if record.source_uri != _SOURCE_URIS["stock_sy_profile_em"]:
        raise ProviderNormalizationError(
            "AKShare goodwill-impairment market-profile source URI does not match "
            "the documented endpoint"
        )
    try:
        upstream_kwargs = _goodwill_impairment_kwargs(
...
def _validate_financial_indicators_normalizer_rows(
    ...
    for row in rows:
        ...
        if row_code != listing.code:
            raise ProviderNormalizationError(
                f"financial-indicator row entity {row_code!r} does not match "
                f"requested listing {listing.canonical_id!r}"
            )
        found, raw_period = _lookup(row, _FINANCIAL_INDICATORS_PERIOD_FIELDS)
        if not found or _text_value(raw_period) in _MISSING_TEXT:
...
def _validate_earnings_quick_report_normalizer_rows(
    ...
    *,
    ...
    """Keep replayed earnings-quick-report rows inside the listing/period."""

    for row in rows:
        row_code = _row_code(row, ListingMarket.A)
        if row_code is None:
            raise ProviderNormalizationError(
                "earnings-quick-report row has no explicit listing code"
            )
        if row_code != listing.code:
            raise ProviderNormalizationError(
                f"earnings-quick-report row entity {row_code!r} does not match "
                f"requested listing {listing.canonical_id!r}"
            )
        found, raw_period = _lookup(row, _EARNINGS_QUICK_REPORT_PERIOD_FIELDS)
        if not found or raw_period is None or _text_value(raw_period) in _MISSING_TEXT:
...
def _validate_shareholder_count_provider_rows(
    ...
    *,
    ...
    """Validate the full CNINFO shareholder-count universe before filtering."""

    for row in rows:
        if _row_code(row, ListingMarket.A) is None:
            raise ProviderResponseError(
                f"AKShare returned a shareholder-count row without a listing code for "
                f"{request.entity_id!r}",
                provider=provider,
                request=request,
            )
...
def _validate_shareholder_count_normalizer_rows(
    ...
    *,
    ...
    """Keep replayed CNINFO shareholder-count rows inside listing/date scope."""

    for row in rows:
        row_code = _row_code(row, ListingMarket.A)
        if row_code is None:
            raise ProviderNormalizationError(
                "shareholder-count row has no explicit listing code"
            )
        if row_code != listing.code:
            raise ProviderNormalizationError(
                f"shareholder-count row entity {row_code!r} does not match "
                f"requested listing {listing.canonical_id!r}"
            )
        found, raw_date = _lookup(row, _SHAREHOLDER_COUNT_DATE_FIELDS)
        if not found or _text_value(raw_date) in _MISSING_TEXT:
...
def _market_activity_month_parameter(
    ...
    *,
    ...
    if not isinstance(raw_value, str) or not re.fullmatch(r"\d{6}", raw_value):
        raise ProviderRequestError(
            f"market-activity {name} must be YYYYMM",
            request=request,
            retryable=False,
        )
    try:
        ...
        raise ProviderRequestError(
            f"market-activity {name} must be a valid YYYYMM month",
            request=request,
            retryable=False,
        ) from exc
...
def _market_activity_szse_area_summary_kwargs(
    ...
    """Build the documented monthly SZSE area-summary request."""

    if endpoint_name != "stock_szse_area_summary":
        raise ProviderRequestError(
            f"unsupported AKShare SZSE area-summary endpoint {endpoint_name!r}",
            request=request,
            retryable=False,
        )
    if listing.market is not ListingMarket.A:
        raise ProviderRequestError(
            "the AKShare SZSE area-summary endpoint supports A-share listings only",
            request=request,
            retryable=False,
        )
    unknown = sorted(
...
def _market_activity_szse_sector_summary_kwargs(
    ...
    """Build the documented monthly SZSE sector-summary request."""

    if endpoint_name != "stock_szse_sector_summary":
        raise ProviderRequestError(
            f"unsupported AKShare SZSE sector-summary endpoint {endpoint_name!r}",
            request=request,
            retryable=False,
        )
    if listing.market is not ListingMarket.A:
        raise ProviderRequestError(
            "the AKShare SZSE sector-summary endpoint supports A-share listings only",
            request=request,
            retryable=False,
        )
    unknown = sorted(
...
def _market_activity_szse_summary_kwargs(
    ...
    """Build the documented date-scoped SZSE market-summary request."""

    if endpoint_name != "stock_szse_summary":
        raise ProviderRequestError(
            f"unsupported AKShare SZSE market-summary endpoint {endpoint_name!r}",
            request=request,
            retryable=False,
        )
    if listing.market is not ListingMarket.A:
        raise ProviderRequestError(
            "the AKShare SZSE market-summary endpoint supports A-share listings only",
            request=request,
            retryable=False,
        )
    unknown = sorted(
...
def _market_activity_sse_deal_daily_kwargs(
    ...
    """Build the documented SSE daily-deal overview request."""

    if endpoint_name != "stock_sse_deal_daily":
        raise ProviderRequestError(
            f"unsupported AKShare SSE daily-deal endpoint {endpoint_name!r}",
            request=request,
            retryable=False,
        )
    if listing.market is not ListingMarket.A:
        raise ProviderRequestError(
            "the AKShare SSE daily-deal endpoint supports A-share listings only",
            request=request,
            retryable=False,
        )
    unknown = sorted(
...
def _market_activity_limit_down_pool_kwargs(
    ...
    """Build the documented date-scoped A-share limit-down-pool request."""

    if endpoint_name != "stock_zt_pool_dtgc_em":
        raise ProviderRequestError(
            "unsupported AKShare market-activity limit-down-pool endpoint "
            f"{endpoint_name!r}",
            request=request,
            retryable=False,
        )
    if listing.market is not ListingMarket.A:
        raise ProviderRequestError(
            "the AKShare market-activity limit-down-pool endpoint supports A-share "
            "listings only",
            request=request,
            retryable=False,
        )
...
def _market_activity_statistic_kwargs(
    ...
    """Build the documented per-stock Dragon-Tiger statistics request."""

    if endpoint_name != "stock_lhb_stock_statistic_em":
        raise ProviderRequestError(
            f"unsupported AKShare market-activity-statistic endpoint {endpoint_name!r}",
            request=request,
            retryable=False,
        )
    if listing.market is not ListingMarket.A:
        raise ProviderRequestError(
            "the AKShare market-activity statistic endpoint supports A-share listings only",
            request=request,
            retryable=False,
        )
...
def _parse_tencent_daily_history_date_parameter(
    ...
    *,
    ...
    if not isinstance(value, str) or not re.fullmatch(r"(?:\d{8}|\d{4}-\d{2}-\d{2})", value):
        raise ProviderRequestError(
            f"AKShare Tencent daily-history {name} must be YYYYMMDD or YYYY-MM-DD",
            request=request,
            retryable=False,
        )
    for fmt in ("%Y%m%d", "%Y-%m-%d"):
        try:
            ...
            continue
    raise ProviderRequestError(
        f"AKShare Tencent daily-history {name} must be a valid date",
        request=request,
        retryable=False,
    )
...
def _intraday_history_kwargs(
    ...
    """Build the documented Eastmoney A-share intraday-history request."""
    ...
    parameters = dict(request.parameters)
    unknown = sorted(set(parameters) - _MARKET_HISTORY_INTRADAY_PARAMETER_NAMES)
    if unknown:
        raise ProviderRequestError(
            "unsupported AKShare intraday-history parameter(s): " + ", ".join(unknown),
            request=request,
            retryable=False,
        )
    if parameters.get("view") != _MARKET_HISTORY_INTRADAY_VIEW:
        raise ProviderRequestError(
            "the AKShare intraday-history endpoint requires "
            f"view={_MARKET_HISTORY_INTRADAY_VIEW!r}",
            request=request,
            retryable=False,
        )

    period = parameters.get("period", _MARKET_HISTORY_INTRADAY_DEFAULT_PERIOD)
    if not isinstance(period, str) or period not in _MARKET_HISTORY_INTRADAY_PERIODS:
        choices = ", ".join(sorted(_MARKET_HISTORY_INTRADAY_PERIODS, key=int))
        raise ProviderRequestError(
            f"AKShare intraday-history period must be one of: {choices}",
            request=request,
            retryable=False,
        )
    adjust = parameters.get("adjust", _MARKET_HISTORY_INTRADAY_DEFAULT_ADJUST)
    if not isinstance(adjust, str) or adjust not in {"", "qfq", "hfq"}:
        raise ProviderRequestError(
            "AKShare intraday-history adjust must be '', 'qfq' or 'hfq'",
            request=request,
            retryable=False,
        )
    ...
    start_date = parameters.get(
        "start_date",
        _MARKET_HISTORY_INTRADAY_DEFAULT_START,
    )
    end_date = parameters.get(
        "end_date",
        _MARKET_HISTORY_INTRADAY_DEFAULT_END,
    )
    start_datetime = _parse_intraday_history_datetime_parameter(
        start_date,
        name="start_date",
    ...
    if start_datetime > end_datetime:
        raise ProviderRequestError(
            "AKShare intraday-history start_date must not be after end_date",
            request=request,
            retryable=False,
        )
    return {
...
_EARNINGS_QUICK_REPORT_PERIOD_FIELDS = (
    "报告日期",
    "报告期",
    "REPORT_DATE",
    "report_date",
...
_FINANCIAL_ABSTRACT_OPTION_FIELDS = ("选项", "option", "category")
_FINANCIAL_ABSTRACT_INDICATOR_FIELDS = ("指标", "indicator", "metric")
_FINANCIAL_INDICATORS_PERIOD_FIELDS = (
    "REPORT_DATE",
    "report_date",
    "报告日期",
    "报告期",
...
def _is_financial_abstract_period_key(value: object) -> bool:
    if not isinstance(value, str):
        return False
    return bool(
...
def _financial_abstract_periods(row: Mapping[str, JSONValue]) -> set[date]:
    return {
        parsed
        for key in row
        if _is_financial_abstract_period_key(key)
        and (parsed := _parse_date_value(key)) is not None
    }


def _financial_indicators_date(row: Mapping[str, JSONValue]) -> date | None:
    found, raw_value = _lookup(row, _FINANCIAL_INDICATORS_PERIOD_FIELDS)
